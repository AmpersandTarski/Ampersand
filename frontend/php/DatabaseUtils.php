...
function DB_doquer($quer) {
  ...
  if ($error)
    die; // throw exception("<div class=InternalError>$error</div>");
  return $result;
}
...
function GetSMF($type)
{
    ...
    // If file is too old, delete it and return false
    $modified = filemtime(SMF_prefix . $type);
    $diffminutes = ( time() - $modified ) / 60;
    if ($diffminutes > 30) 
    {
        unlink(SMF_prefix . $type);
        return false;
    }
