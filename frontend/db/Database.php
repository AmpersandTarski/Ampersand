...
if (!isset($execEngineSays))    // should have been set in 'config.php'
   $execEngineSays = true;      // When 'true' the ExecEngine provides high level logging
if (!isset($execEngineWhispers))// should have been set in 'config.php'
...
{ global $allRoles;
  ...
  {  // execEngineSays("Skipped $ruleName"); 
    ...
    $rows = DB_doquer($ruleSql['violationsSQL']); // execute violationsSQL to check for violations
...
{ 
  ...
  { foreach ($allRoles as $role)
    ...
    { 
        ...
        $rows = DB_doquer($ruleSql['violationsSQL'], $error); // execute violationsSQL to check for violations
        if ($error) error("While evaluating rule '$ruleName': ".$error);
...
{  echo "
  ...
  <strong>Violation!</strong> <a href=\"#\" class=\"alert-link\">".$msg."</a></div>";
}
