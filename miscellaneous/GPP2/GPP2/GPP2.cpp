...
void newmacro(char *s,int len,int hasspecs)
{
  ...
  macros[nmacros].username=(char *)malloc(len+1);
...
void parseCmdlineDefine(char *s)
{
  ...
  if (s[l]=='(') {
    ...
    do {
      ...
      if (i>l) {
        macros[nmacros].argnames[argc-1]=(char *)malloc(i-l+1);
...
void outchar(char c)
{
  if (C->out->bufsize) {
    if (C->out->len+1==C->out->bufsize) { 
      ...
      C->out->buf=(char *)realloc(C->out->buf,C->out->bufsize);
...
void initthings(int argc,char **argv)
{
  ...
  C->bufsize=80;
  C->len=0;
  C->buf=C->malloced_buf=(char *)malloc(C->bufsize);
  C->eof=0;
  C->namedargs=NULL;
  C->in_comment=0;
...
char *ProcessText(char *buf,int l,int ambience)
{
  ...
  if (l==0) { s=(char *)malloc(1); s[0]=0; return s;  }
  s=(char *)malloc(l+2);
  s[0]='\n';
  memcpy(s+1,buf,l);
  s[l+1]=0;
...
char *ArithmEval(int pos1,int pos2)
{
  ...
  if (i>=0) warning("the defined(...) macro is already defined");
    ...
    macros[nmacros].macrotext=(char *)malloc(1);
  ...
  t=(char *)malloc(MAX_GPP_NUM_SIZE);
  sprintf(t,"%d",i);
  free(s);
  return t;
}
...
int ParsePossibleMeta()
{
  ...
  case 1: /* DEFINE */
    if (!commented[iflevel]) {
      ...
      for (j=0;j<argc;j++) {
        ...
        macros[nmacros].argnames[j]=(char *)malloc(arge[j]-argb[j]+1);
  ...
  case 7: /* INCLUDE */
	...
	{ p1start++; p1end--; }
      ...
      incfile_name=(char *)malloc(p1end-p1start+1);
	...
	incfile_name=(char *)realloc(incfile_name,p1end-p1start+1);
	/* extract the orig include filename */
	for (i=0;i<p1end-p1start;i++) 
	  incfile_name[i]=getChar(p1start+i);
	incfile_name[p1end-p1start]=0;
...
int ParsePossibleUser()
{
  ...
  if ((macros[id].nnamedargs==-1)&&(lg_end>=0)&&
    ...
    C->buf=C->malloced_buf=(char *)malloc(l);
    ...
    C->buf=C->malloced_buf=(char *)malloc(strlen(macros[id].macrotext)+2);
    C->buf[0]='\n';
    strcpy(C->buf+1,macros[id].macrotext);
  }
  C->len=strlen(C->buf);
...
void escape_backslashes(char *instr, char **outstr)
{
  ...
  *outstr =(char *) malloc(2*strlen(instr));
					...
					char *includemarker_input)
{
  ...
  *include_directive_marker = (char *)malloc(len+18);
