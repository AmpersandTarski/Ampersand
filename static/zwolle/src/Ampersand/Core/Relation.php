...
class Relation {
    ...
    public function deleteLink($leftAtom, $rightAtom, $isFlipped = false, $source = 'User'){
        ...
        // Checks
        if(!$this->srcConcept->inSameClassificationTree($srcAtom->concept)) throw new Exception ("Cannot delete link ({$srcAtom->__toString()},{$tgtAtom->__toString()}) from relation '{$this->__toString()}', because source concept does not match relation source.", 500);
        if(!$this->tgtConcept->inSameClassificationTree($tgtAtom->concept)) throw new Exception ("Cannot delete link ({$srcAtom->__toString()},{$tgtAtom->__toString()}) from relation '{$this->__toString()}', because target concept does not match relation target.", 500);
