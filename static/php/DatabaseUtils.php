...
function getPopulationADL() {
  ...
  $adl  = "CONTEXT PopulationDump IN ENGLISH\n"
        . "-- Population dump for database '$dbName' on $date\n"
        . "-- Generated by $versionInfo.\n\n";
  foreach (array_keys($conceptTableInfo) as $concept) {
    if ($concept != "ONE") {       
      $conceptEsc = escapeAndDblQuote($concept);
      $adl .= "POPULATION $conceptEsc CONTAINS ".showAtomListADL(getAllConceptAtoms($concept))."\n";
    }
  }
  foreach ($relationTableInfo as $relationInfo) {
    $srcEsc = escapeAndDblQuote($relationInfo[srcConcept]);
    $tgtEsc = escapeAndDblQuote($relationInfo[tgtConcept]);
...
function showPairADL($pair) {
  return '('.escapeAndDblQuote($pair['src']).','.escapeAndDblQuote($pair['tgt']).')';
}
...
function escapeAndDblQuote($str) {
  return '"'.addslashes($str).'"';
}
