<?
  function checkRule1(){
    // No violations should occur in (leadsto~;kind -: kind COMPUTING [kind])
     ...
     $v=DB_doquer('
                  SELECT DISTINCT vfi.AttD_ecision, vfi.AttP_roduct
                    FROM 
                      ( 
                   ...
                   WHERE NOT EXISTS (
                              SELECT *
                                FROM T7_kind AS cp
                               WHERE vfi.AttD_ecision=cp.AttD_ecision AND vfi.AttP_roduct=cp.AttP_roduct)');
     if(count($v)) {
  ...
  function checkRule2(){
    // No violations should occur in (assigned -: applicant;inhabitant;area~)
     ...
     $v=DB_doquer('
                  SELECT DISTINCT vfi.AttA_pplication, vfi.AttE_mployee
                    FROM T4_assigned AS vfi
                   WHERE NOT EXISTS (
                              SELECT *
                                FROM 
                                  ( 
                                    SELECT DISTINCT fst.AttA_pplication, snd.AttE_mployee
                                      FROM T2_applicant AS fst, 
                                        ( 
                                          SELECT DISTINCT fst.AttP_erson, snd.AttE_mployee
                                            FROM T8_inhabitant AS fst, T9_area AS snd
                                           WHERE fst.AttA_rea = snd.AttA_rea
                                        ) AS snd
                                     WHERE fst.AttP_erson = snd.AttP_erson
                                  ) AS cp
                               WHERE vfi.AttA_pplication=cp.AttA_pplication AND vfi.AttE_mployee=cp.AttE_mployee)');
     if(count($v)) {
  ...
  function checkRule3(){
    // No violations should occur in (applicant -: checked;authentic~)
     ...
     $v=DB_doquer('
                  SELECT DISTINCT vfi.AttA_pplication, vfi.AttP_erson
                    FROM T2_applicant AS vfi
                   WHERE NOT EXISTS (
                              SELECT *
                                FROM 
                                  ( 
                               ...
                               WHERE vfi.AttA_pplication=cp.AttA_pplication AND vfi.AttP_erson=cp.AttP_erson)');
     if(count($v)) {
  ...
  function checkRule5(){
    // No violations should occur in (authentic;authentic~ -: I[Person])
     ...
     $v=DB_doquer('
                  SELECT DISTINCT vfi.AttP_erson, vfi.AttP_erson1
                    FROM 
                      ( 
                   ...
                   WHERE NOT EXISTS (
                              SELECT *
                               ...
                               WHERE vfi.AttP_erson=cp.AttP_erson AND vfi.AttP_erson1=cp.AttP_erson1)');
     if(count($v)) {
  ...
  function checkRule6(){
    // No violations should occur in (I[IDdocument] -: authentic~;authentic)
     ...
     $v=DB_doquer('
                  SELECT DISTINCT vfi.AttID_document, vfi.AttID_document1
                    FROM 
                      ( 
                        SELECT DISTINCT AttID_document, AttID_document AS AttID_document1
                          FROM C2_ID_document
                         WHERE 1
                      ) AS vfi
                   WHERE NOT EXISTS (
                              SELECT *
                               ...
                               WHERE vfi.AttID_document=cp.AttID_document AND vfi.AttID_document1=cp.AttID_document1)');
     if(count($v)) {
  ...
  function checkRule9(){
    // No violations should occur in (I[Application] -: applicant;applicant~)
     ...
     $v=DB_doquer('
                  SELECT DISTINCT vfi.AttA_pplication, vfi.AttA_pplication1
                    FROM 
                      ( 
                        SELECT DISTINCT AttA_pplication, AttA_pplication AS AttA_pplication1
                   ...
                   WHERE NOT EXISTS (
                              SELECT *
                               ...
                               WHERE vfi.AttA_pplication=cp.AttA_pplication AND vfi.AttA_pplication1=cp.AttA_pplication1)');
     if(count($v)) {
  ...
  function checkRule10(){
    // No violations should occur in (assigned~;assigned -: I[Employee])
     ...
     $v=DB_doquer('
                  SELECT DISTINCT vfi.AttE_mployee, vfi.AttE_mployee1
                    FROM 
                      ( 
                   ...
                   WHERE NOT EXISTS (
                              SELECT *
                                FROM 
                                  ( 
                                    SELECT DISTINCT AttE_mployee, AttE_mployee AS AttE_mployee1
                                      FROM C4_E_mployee
                                     WHERE 1
                                  ) AS cp
                               WHERE vfi.AttE_mployee=cp.AttE_mployee AND vfi.AttE_mployee1=cp.AttE_mployee1)');
     if(count($v)) {
                   ...
                   WHERE NOT EXISTS (
                              SELECT *
                                FROM 
                                  ( 
                                    SELECT DISTINCT fst.AttE_mployee, snd.AttE_mployee AS AttE_mployee1
                                      FROM T5_auth AS fst, T5_auth AS snd
                                     WHERE fst.AttP_roduct = snd.AttP_roduct
                                  ) AS cp
                               WHERE vfi.AttE_mployee=cp.AttE_mployee AND vfi.AttE_mployee1=cp.AttE_mployee1)');
     if(count($v)) {
  ...
  function checkRule13(){
    // No violations should occur in (I[Product] -: auth~;auth)
     ...
     $v=DB_doquer('
                  SELECT DISTINCT vfi.AttP_roduct, vfi.AttP_roduct1
                    FROM 
                      ( 
                        SELECT DISTINCT AttP_roduct, AttP_roduct AS AttP_roduct1
                          FROM C5_P_roduct
                         WHERE 1
                      ) AS vfi
                   WHERE NOT EXISTS (
                              SELECT *
                               ...
                               WHERE vfi.AttP_roduct=cp.AttP_roduct AND vfi.AttP_roduct1=cp.AttP_roduct1)');
     if(count($v)) {
  ...
  function checkRule17(){
    // No violations should occur in (I[Decision] -: kind;kind~)
     ...
     $v=DB_doquer('
                  SELECT DISTINCT vfi.AttD_ecision, vfi.AttD_ecision1
                    FROM 
                      ( 
                        SELECT DISTINCT AttD_ecision, AttD_ecision AS AttD_ecision1
                          FROM C6_D_ecision
                         WHERE 1
                      ) AS vfi
                   WHERE NOT EXISTS (
                              SELECT *
                               ...
                               WHERE vfi.AttD_ecision=cp.AttD_ecision AND vfi.AttD_ecision1=cp.AttD_ecision1)');
     if(count($v)) {
  ...
  function checkRule18(){
    // No violations should occur in (I[Area] -: inhabitant~;inhabitant)
     ...
     $v=DB_doquer('
                  SELECT DISTINCT vfi.AttA_rea, vfi.AttA_rea1
                    FROM 
                      ( 
                        SELECT DISTINCT AttA_rea, AttA_rea AS AttA_rea1
                          FROM C7_A_rea
                         WHERE 1
                      ) AS vfi
                   WHERE NOT EXISTS (
                              SELECT *
                               ...
                               WHERE vfi.AttA_rea=cp.AttA_rea AND vfi.AttA_rea1=cp.AttA_rea1)');
     if(count($v)) {
