...
Ampersand also generates a formal description of the API in the OpenAPI standard, so that other
software can discover and use it. What you can do with that is discussed below.
...
## Connecting other software through the API

Because an Ampersand application exposes its functionality through an API, it can act as a building block in a larger software landscape instead of as an isolated end-user application. To make that API easy to use, the Ampersand compiler produces a formal, machine-readable description of it in the widely supported [OpenAPI](https://www.openapis.org/) standard. This description lists every interface as an operation on the API — what can be read, created, updated and deleted — together with the structure of the data that goes in and out. You do not write this description by hand; it comes out of the compiler along with the rest of the application.

Having such a description available unlocks a range of applications that would otherwise take considerable effort to build:

- **Living documentation.** Interactive API documentation (for example through Swagger UI or Redoc) can be rendered directly from the description, giving developers an always-current reference and a place to try calls out.
- **Client code in any language.** Code generators turn the description into ready-to-use client libraries (SDKs), so another application can talk to your Ampersand system without hand-written plumbing.
- **Integration with other systems.** Integration platforms, API gateways and many low-code tools understand OpenAPI, which makes it straightforward to connect an Ampersand application to the systems already in use in an organisation.
- **Automated testing and mocking.** Testing tools can use the description to check that the running API behaves as promised, and mocking tools can stand in for the API while other software is being developed.

The decisive benefit is that this description is generated from the same Ampersand script as the application itself. It therefore always matches the running system: when the model changes, the description changes with it. There is no separate API document to keep in step, which removes a common source of mistakes and maintenance work. In this way the OpenAPI description extends the correctness that Ampersand guarantees for the data model into a dependable contract for every other system that connects to it.
