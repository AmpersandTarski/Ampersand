...
--    -> interfaces harvest (read) and distribute (edit) data from and to [Plug]
-- 2) Rules within the scope of a data object can and will be maintained in the interface.
--    -> (Composite) interfaces get their data (unpredicatable which data) from one or more data objects.
--       From an interface it is complex to determine whether you may commit (e.g. are all requiredFields for this edit action available)
--       A data object has implemented the rules that determine just that.
--       Separate data and composite interfaces is a solution.
--       Composite interfaces maintain rules that are not within the scope of any data object.
...
instance Object DataObject where
 ...
 attributes (DataObject p@(TblSQL{}))
  = if length mbfld==1 
    ...
    else fatal 59 "cannot find field of dataobject"
    where mbfld = [fld |(c,fld)<-cLkpTbl p, c==concept p]
 contextOf (DataObject p@(ScalarSQL{}))
  = fldexpr (column p)
 contextOf (DataObject p@(BinSQL{}))
  = fldexpr (fst(columns p))
 contextOf (DataObject p@(TblSQL{}))
  = if length mbexpr==1 
    then head mbexpr 
    else fatal 68 "cannot find context of dataobject"
    where mbexpr = [fldexpr fld |(c,fld)<-cLkpTbl p, c==concept p] 
 --TODO -> (see tblcontents)

dobj2objats::String->SqlField->[(Relation,SqlField,SqlField)]->[ObjectDef]
dobj2objats _ fld mlkp = objats(fld2objdef fld (fld2objats fld mlkp))
fld2objdef::SqlField->[ObjectDef]->ObjectDef
fld2objdef fld ats = Obj (fldname fld) (Origin "object generated from field by fld2objdef") (fldexpr fld) ats []
fld2objats::SqlField->[(Relation,SqlField,SqlField)]->[ObjectDef]
...
-- Usually source a==concept p. Otherwise, the attribute computation is somewhat more complicated. See ADL2Fspec for explanation about kernels.
 attributes p@(TblSQL{})
  = [ Obj (fldname tFld)                                                   -- objnm 
          ...
          (if source a==concept p then ERel a  else f (source a) [[a]])  -- objctx
          [] []                                                            -- objats and objstrs
    | (a,_,tFld)<-mLkpTbl p]
    where
     f c mms
       ...
       | otherwise = ECps [ERel r | r <- head (sort' length stop)]  -- pick the shortest path and turn it into an expression.
      where
        mms' = if [] `elem` mms 
 ...
 contextOf p@(BinSQL{}) = ERel (mLkp p) 
...
plugpath p@(BinSQL{}) srcfld trgfld
  | srcfld==trgfld = let tm=ERel (mLkp p) --(note: mLkp p is the relation from fst to snd column of BinSQL)
