...
module DatabaseDesign.Ampersand.Output.AdlExplanation (purpose, Meaning(..))
where
...
class  Identified a => Explainable a where 
--  meaning :: Lang -> a -> [Block]             -- ^ explains the meaning of a. The meaning is preferrably specified by the user. If not, Ampersand tries to generate a meaning in natural language.
  purpose :: Fspc -> Lang -> a -> Explanation -- ^ explains the purpose of a, i.e. the reason why a exists. The purpose could be either given by the user, or generated by Ampersand. 
  purpose fSpec l x = case expls of
                        []   -> fatal 40 "No purpose is generated! (should be automatically generated and available in Fspc.)"
                        [e]  -> e
                        e:es -> fatal 42 ("Too many purposes given for " ++show (name x))
...
instance Explainable Activity where
  purpose _ l x = case expls of
                    []   -> fatal 212 "No purpose is generated! (should be automatically generated and available in Fspc.)"
                    [e]  -> e
                    _    -> fatal 214 ("Too many purposes given for " ++show (name x))
      where
       expls = [ e | e <- actXpls x, amLang (explMarkup e) == l ]
...
instance Meaning Rule where
  meaning l r = case filter isLang (rrmean r) of
...
instance Meaning Declaration where
  meaning l d = case filter isLang (decMean d) of
