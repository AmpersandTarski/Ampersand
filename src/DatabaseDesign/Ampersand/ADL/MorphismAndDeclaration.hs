...
module DatabaseDesign.Ampersand.ADL.MorphismAndDeclaration (Relation(..),Association(..),Relational(..), mapRelation
...
--   isIdent        :: r -> Bool  -- > tells whether the argument is equivalent to I
  ...
  --  singleton e     = isProp e && isTrue e
   ...
   data Relation c = 
                   Rel  { relnm :: String            -- ^ the name of the relation. This is the same name as
                        ...
                        , relpos :: FilePos          -- ^ the position of the rule in which the relation occurs
                        ...
                        , relyin :: Bool             -- ^ the 'yin' factor. If true, a declaration is bound in the same direction as the relation. If false, binding occurs in the opposite direction.
                        , reldcl :: Declaration c    -- ^ the declaration bound to this relation.
                  ...
                  | Mp1 { rel1val :: String          -- ^ the value of the one relation
   ...
   mapRelation :: Eq a => (a->b) -> Relation a -> Relation b
   mapRelation f m@Rel{} = m{ relats = map f (relats m)
                            , relsrc = f (relsrc m)
                            , reltrg = f (reltrg m)
                            , reldcl = mapDeclaration f (reldcl m)
                            }
   mapRelation f m@I  {} = m{ relats = map f (relats m)
                            , relspc = f (relspc m)
                            , relgen = f (relgen m)
                            }
   mapRelation f m@V  {} = m{ relats = map f (relats m)
                            , reltyp = let (s,t)=sign m in (f s, f t)
                            }
   mapRelation f m@Mp1{} = m{ relats  = map f (relats m)
                            , rel1typ = f (rel1typ m)
                            }
...
-- \***********************************************************************
 ...
 --   m == m' = name m==name m' && source m==source m' && target m==target m' && yin==yin'
   ...
   instance (Eq c) => Association (Relation c) c where
    sign   m@Rel{}               = (source m, target m)    -- BECAUSE: (source m, target m) represents the actual type of this relation. Yin takes care of the consistency with the underlying declaration, reldcl m.
...
-- \***********************************************************************
   ...
   instance (Identified c,Show c) => Show (Declaration c) where
    showsPrec _ d
     = showString (intercalate " " ([decnm d,"::",name (desrc d),"*",name (detrg d),show (decprps_calc d),"PRAGMA",show (decprL d),show (decprM d),show (decprR d)]))
