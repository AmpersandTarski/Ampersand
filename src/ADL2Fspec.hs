  ...
  where
   ...
   import Adl
   import Dataset
   import Auxiliaries    (naming, eqCl, sort')
   import FspecDef
   import Languages
 ...
 -- Every theme will be explained in a chapter of its own.
   ...
   makeFspec flags context =
      Fspc { fsfsid = makeFSid1 (name context)
        ...
        newplugs = uniqueNames forbiddenNames
        ...
        allplugs = uniqueNames forbiddenNames (relPlugs ++ map conc2plug looseConcs)
          where
           otherRels      = looseRels >- mors definedplugs
           looseRels      = map makeMph (declarations context) >- mors definedplugs
           looseConcs     = concs (declarations context) >- concs (definedplugs ++ relPlugs)
...
{- genPlugs computes a set of SQL plugs to obtain wide tables with minimal redundancy.
   First, we define a plug for each concept.   Code:   [ makePlug c | c<-concs context]
   Secondly, we sort the plugs on length, the longest first. Code:   sort' ((0-).length.fields)
   Finally, we filter out all shorter plugs that can be represented by longer ones. Code: absorb
-}
   genPlugs :: Context -> [Plug]
   genPlugs context
    = absorb (sort' ((0-).length.fields) [ makePlug c | c<-concs context])
      where
       makePlug :: Concept -> Plug
       {- makePlug produces a plug with kernel c, and SQL-fields around it.
          It is meant to become a table in SQL for c, with each field being one attribute.
       -}
       makePlug c
        = PlugSql { fields = uniqueNames (out [mIs c] (name (mIs c)): recur (out,cond) c )
                  , plname = name c
                  }
          where out ms nm
                 = Fld { fldname = nm
                       , fldexpr = disjNF (F [Tm m'| m'<-ms])
                       , fldtype = if and [isIdent m|m<-ms] then SQLId else SQLVarchar 255
                       , fldnull = not (Tot `elem` multiplicities expr)      -- can there be empty field-values? 
                       , flduniq = True                                      -- are all field-values unique?
                       , fldauto = isIdent (head ms) }                       -- is the field auto increment?
                   where expr = disjNF (F [Tm m'| m'<-ms])
                cond m = not (Inj `elem` multiplicities m) && Uni `elem` multiplicities m

{- relsFrom
computes all relations that are directly adjacent to c
-}
       relsFrom :: Concept -> [Morphism]
       relsFrom c = [m | m<-ms, source m == c]
         where ms = [makeMph d  | d@(Sgn {})<-ds]++[flp (makeMph d) | d@(Sgn {})<-ds]
               ds = [ d | d@(Sgn {})<-declarations context, not (isSignal d)]

{- closure computation
The closure of condition is computed and a list of outputs is produced, each output representing a path in the relation graph.
-}
       recur (out,condition) c = rec [] c
        where rec ms c'
               = [ out (ms++[m]) (name m)
                 | m<-relsFrom c', condition m, not (m `elem` ms) ] ++
                 [ sf | m<-relsFrom c', condition m, not (m `elem` ms), sf<-rec (ms++[m]) (target m) ]
...
-}
       absorb []     = []
       absorb (p:ps) = p: absorb [p'| p'<-ps, not (kernel p' `elem` recur (out,cond) (kernel p))]
        where
         out ms _ = source (head ms)
         cond m = Sur `elem` multiplicities m  -- the closure of this condition is computed.
