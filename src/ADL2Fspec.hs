    ...
    where
        ...
        fSpec =
            Fspc { fsName       = if genPrototype flags 
                 ...
                 , plugInfos    = allplugs
                 , serviceS     = ctxsvcs context -- services specified in the Ampersand script
                 , serviceG     = [svc| svc<-serviceGen, let ctxrel = objctx (svObj svc)
        ...
        relPlugs = [ rel2plug rel totals --(see rel2plug in Plug.hs)
                   | rel<-savedRels
                   , not (Inj `elem` multiplicities rel)
                   , not (Uni `elem` multiplicities rel)]
...
-}
        ...
        --making services
        -------------------
-- WHY don't we use the gplugs that have already been generated? In that way we can be sure that the services
...
--  Ampersand generates services for the purpose of quick prototyping.
--  A script without any mention of services is supplemented
...
--                                       thus, no CON for contexts with only signals 
        ...
        step4a
         ...
         = let cptsvcs = [(c,cfld,p)| InternalPlug p@TblSQL{}<-allplugs, (c,cfld)<-cLkpTbl p]
           ...
           [Serv { svName   = name c
                 , svParams = [makeInline rel| rel<-mors p, not (isIdent rel)]
                 , svViols  = []
                 , svArgs   = []
                 , svObj    = Obj { objnm   = name c                                                  
                                  , objpos  = Nowhere                                                 
                                  ...
                                  , objstrs = [] }                                                    
                 , svPos    = Nowhere
                 , svExpl   = "Service " ++name c++" has been generated by Ampersand."
                 }
           | (c,cfld,p)<-cptsvcs] 
         --end student theme
         ...
         = let recur trace es
           ...
           in
           [Serv { svName   = name c
                 , svParams = [makeInline rel| rel<-mors objattributes, not (isIdent rel)]
                 , svViols  = []
                 , svArgs   = []
                 , svObj    = Obj { objnm   = name c
                                  , objpos  = Nowhere
        ...
        step4b --generate lists of concept instances for those concepts that have a generated SERVICE in step4a 
         = [Serv { svName   = nm
                 , svParams = svParams svcc
                 , svViols  = svViols  svcc
                 , svArgs   = svArgs   svcc
                 , svObj    = Obj { objnm   = nm
                                  , objpos  = Nowhere
                                  , objctx  = Tm (mIs S)(-1)
           ...
           | svcc<-step4a
           , let c   = source(objctx (svObj svcc))
                 nm  = plural (language flags)(name c)
                 att = Obj (name c) Nowhere (Tm (V [cptS,c] (cptS,c))(-1)) Nothing [] []
           ]
        ----------------------
        --END: making services
        ----------------------
   ...
   makeFservice :: Context -> [Quad] -> Service -> Fservice
   makeFservice context _ svc
    = let s = Fservice{ fsv_svcdef = svc  -- the service from which this fservice is drawn
...
-- All concepts of which this service can delete instances
                      ...
                      , fsv_fpa       = case depth (svObj svc) of -- Valideren in de FPA-wereld
                                          0 -> NO
                                          1 -> IF Eenvoudig
                                          2 -> IF Eenvoudig
                                          3 -> IF Gemiddeld
...
--        signalRs    = preEmpt (assembleECAs visible (quads visible signalInvs))
        ...
        srvfields = [fld 0 o| o<-objats (svObj svc)]
