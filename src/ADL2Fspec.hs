...
module ADL2Fspec (makeFspec,actSem, delta, allClauses, conjuncts, quads, assembleECAs, preEmpt, doCode, editable, editMph)
  where
   ...
   import Data.Plug
   import Data.ADL2Plug  (makeSqlPlug,makeTblPlugs,mor2plug)
   import ShowADL
   import FPA
   ...
   makeFspec flags context = fSpec
    where
        ...
        gPlugs :: [PlugSQL]
        gPlugs   = makeTblPlugs context savedDecs [p|InternalPlug p<-vsqlplugs]
...
--                                       thus, no CON for contexts with only signals 
        ...
        step4a
         ...
         = let cptsvcs = [(c,cfld,p)| InternalPlug p@(TblSQL{})   <-allplugs, (c,cfld)<-cLkpTbl p]
               ...
               --atts of cfld
               myatts c = [attfld| InternalPlug p@(TblSQL{})   <-allplugs, (kfld',attfld)<-attrels p, target(fldexpr attfld)==c]
