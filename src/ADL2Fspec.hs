  ...
  where
   ...
   makeFspec flags context = fSpec
    where
        ...
        definedplugs = map makePlug vsqlplugs ++ map makePlug vphpplugs
...
-- Univalent relations and injective relations cannot be associations, because they are used as attributes in wide tables.
        ...
        allplugs = definedplugs ++      -- all plugs defined by the user
                   map makePlug
                       ( uniqueNames (pickTypedPlug definedplugs)  -- the names of definedplugs will not be changed, assuming they are all unique
                          (gPlugs ++ -- all plugs generated by the compiler
                           relPlugs)            -- all plugs for relations not touched by definedplugs and gplugs
                       )
          where
           gPlugs :: [PlugSQL]
           gPlugs   = makePlugs context savedDecs (pickTypedPlug definedplugs)
           relPlugs :: [PlugSQL]
        ...
        uniqueNames :: [PlugSQL]->[PlugSQL]->[PlugSQL]
        ...
        uniqueNames taken plgs
         = [p | cl<-eqCl (map toLower.plname) plgs  -- each equivalence class cl contains plugs p with the same map toLower (name p)
              , p <-if plname (head cl) `elem` map plname taken || length cl>1
                    then [renamePlug p (plname p++show i)| (p,i)<-zip cl [(1::Int)..]]
                    else cl
           ]
