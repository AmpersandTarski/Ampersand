   ...
   makeFspec flags context = fSpec
    where
        ...
        allplugs = definedplugs ++      -- all plugs defined by the user
                   map makePlug
                       ( uniqueNames
                          (pickTypedPlug definedplugs) -- the names of definedplugs will not be changed, assuming they are all unique
                          (gPlugs ++ relPlugs)
                       )
        -- all plugs generated by the compiler
        gPlugs :: [PlugSQL]
...
-}
        ...
        --TODO151210 -> I'll bet this is a function for any Identified data type
...
--          and generate V[ONE*Concept] for each concept
        ...
        scalarPlugs = [makePlug p|p@(ScalarSQL{})<-pickTypedPlug allplugs]
