...
--copied from Classification rev:195
...
anal flags fSpec
 ...
 --  writing the large rhs frame (concept) with empty contents
      writeFile (filenm "Concept.html") (htmlPage "Concept" leader (htmlBody introtext))               >>
      putStr ("Concept.html written,\n")                                                      >>
 --  writing context switching (top left)
  --TODO    writeFile ("CTX_"++fnContext context++".html") (contextFrame (Cl context world))        >>
    --TODO  putStr("HTML code for context tree "++fnContext context++".html written\n")             >>
 --  writing the main html page, containing the required frames
      writeFile (filenm "index.html") (indexcode (fnFspec fSpec++".html"))                         >>
 ...
 --TODO     sequence_ [navigators cTrees c predLogic graphicstyle| c<-preCl (Cl context world)]     >>
          ...
          sequence_ [ sequence_ [writeFile ("NAV_"++fnPatConcept context pat c++".html")
           ...
           where
            cdSpecs context fnm full b
             = --TODO -> replace with classdiagram2dot
               --writeFile (fnm++"_CD.dot") (cdDataModel context full "dot" b)      >>
               putStrLn ("DISABLED: Class diagram "++fnm++"_CD.dot written... ")  --    >>
               --processCdDataModelFile  (fnm ++"_CD")
            graphSpecs fnm b
             = --TODO -> replace with pattern2dot
   ...
   writeFile ("ANAL"++htmlname (name context)++".html")
             (htmlPage ("Analysis of "++name context) ""
                       ( htmlBody (htmlHeadinglevel 1 ("Analysis of "++name context) []++"\n"++
                                   ...
                                   if null atlasviolations then "" else
                                   htmlHeadinglevel 2 ("Not all rules are satisfied in context "++name context) []++"\n"++
                                   atlasviolations
             )         )          )
 where
   atlasviolations = concat [ hv r| r<-rules context]++
...
genRel context s
     ...
     where a  = name (source s)
           ...
           nijssenZin (Sgn _ _ _ _ l s r (c:cs) expla _ _ _)
            = expla++"A tuple, for example <I>("++fst c++","++snd c++")</I> in the following table means:<BR />"++l++fst c++s++snd c++r++".<P>"
           nijssenZin (Isn g s)
...
htmlRule context r predLogic
 ...
 {-                             ++(if length (atlasviolations r)>1 then "Analysis of the first instance:<BR />\n" else "")++
                                (if null antcStrands
 ...
 where ruleviol    = htmlViolations r
       (x,y)       = if null vs then error ("No atlasviolations in "++show (atlasviolations r)++".") else
                     head vs where vs = atlasviolations r
       nullN (x:xs)= length [e| e<-x, not (null e)]>1
       nullN []    = False
       gE          = genE r
htmlViolations :: Rule -> String
htmlViolations r
 = if null vs then "" else -- No atlasviolations detected within this rule.\n
   htmlBlueTable "red" [show (source r),show (target r)] (map paire2list vs)++"\n\n"
   where vs = atlasviolations r
atlasviolations :: Rule -> [Paire]
atlasviolations r | isSignal r = []
...
multViolations s Uni = [rec| cl<-eqCl fst (contents s), length cl>1, rec<-cl]
multViolations s Inj = [rec| cl<-eqCl snd (contents s), length cl>1, rec<-cl]
...
viewpoint context c
 = Pat (name c)
       rulesV
       (clearG [G Nowhere g s| Isa ts ss<-[isa context], (g,s)<-ts, g `elem` concsV, s `elem` concsV])
       (declarations rulesV)
...
inhViewpoint (Cl context world) specific gen
 = Pat ("Concept "++name specific++" inherited from "++name gen)
       ...
       (clearG [G Nowhere g s| Isa ts ss<-[isa context], (g,s)<-ts, g==gen, s `elem` concs rs])
       (declarations rs)
