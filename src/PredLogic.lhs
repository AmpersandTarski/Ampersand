...
The following function prints a PredLogic in OO notation.
...
>                    implies antc cons = "IF "++antc++" THEN "++cons
  ...
  objOrShow (Forall vars restr)
   = langVars English "For all" vars ++ objOrShow restr
  objOrShow (Exists vars restr)
   = langVars English "Exists" vars  ++ objOrShow restr
  objOrShow (Implies antc conseq)
   = "IF "++objOrShow antc++" THEN "  ++ objOrShow conseq
  objOrShow (Equiv lhs rhs)
   = objOrShow lhs++" = "   ++ objOrShow rhs
  objOrShow (Conj rs)
   = if null rs then "" else
     chain " AND " (map objOrShow rs)
  objOrShow (Disj rs)
   = if null rs then "" else
     chain " OR " (map objOrShow rs)
  objOrShow (Not rs) = "NOT "++objOrShow rs
...
Testje voor dagger: 
nav -: bezoekrapport~ ; verzondenAan- ! veroorzaker~
