...
fSpec2UML fSpec opts = 
 do { packageId0 <- mkUnlabeledId "TopPackage"
    ...
    ; requirementsUML <- mapM genUMLRequirement (requirements opts fSpec)
    ; diagramElements <- genDiagramElements 
    ; customProfileElements <- genCustomProfileElements
    ; customReqElements <- genCustomReqElements opts packageId2
 ...
 where classDiag = cdAnalysis fSpec opts
...
genCustomReqElements opts parentPackageId =
 do { reqVals <- gets reqValues
    ; return [reqUML req | req <- reverse reqVals]
    }
  where
    ...
    reqUML (xmiId, req) = intercalate "\n"
     ([ "    <element xmi:idref="++show xmiId++" xmi:type=\"uml:Requirement\" name="++show (reqId req)++" scope=\"public\""++">"
      ...
      , "      <properties documentation="++show (maybe "" aMarkup2String (meaning (language opts) req))++" isSpecification=\"false\" sType=\"Requirement\" nType=\"0\" scope=\"public\" stereotype=\"Functional\"/>"
      , "      <tags>"]++
