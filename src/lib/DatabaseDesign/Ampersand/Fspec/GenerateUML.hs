...
fSpec2UML fSpec flags = 
 do { packageId0 <- mkUnlabeledId "TopPackage"
    ...
    ; customReqElements <- genCustomReqElements fSpec flags packageId2
...
genCustomReqElements :: Fspc -> Options -> String -> UML
genCustomReqElements fSpec _ parentPackageId =
 do { reqVals <- gets reqValues
    ; return [reqUML req | req <- reverse reqVals]
    }
  where
