...
>      ["\\title{Conceptuele Analyse\\\\"++cname++"}"
>      , "\\maketitle"
>      , "\\tableofcontents"
>      , latexChapter "Inleiding" "Inleiding"
>      , "\tDit document analyseert kernbegrippen in de context van "++name ctx++"."
>      , "\tDe principes, op basis waarvan de analyse is gemaakt staan beschreven in hoofdstuk \\ref{chp:Principes}."
>      , "\tEen kennismodel volgt in hoofdstuk \\ref{chp:Kennisstructuur}."
>      , "\tDit model is bruikbaar als beginpunt voor het gegevensmodel voor het stelsel van applicaties, dat "++name ctx++" ondersteunt.\n"
>      , "\tDit document is een samenvatting van ontologische analyses, gemaakt in Ampersand.\n"
>      , "\tEen uitgewerkte versie van de gehele analyse is op aanvraag beschikbaar."
>      , "\tImplementatie van het kennismodel zoals in deze studie is gedefinieerd, garandeert de bouwbaarheid van de in hoofdstuk \\ref{chp:Principes} genoemde principes."
>      , latexChapter "Principes" "Principes"
>      , "\tDit hoofdstuk bespreekt een aantal principes, waarop de conceptuele analyse voor "++cname++" is gebaseerd."
>      , "Er bestaan twee soorten principes: regels die worden gehandhaafd en actieregels."
>      , "Een actieregel, herkenbaar aan de zinsnede \"Actie nemen\","
>      , "betekent dat de informatiesystemen een signaal moeten generen die een actie initieert of een menselijke actie agendeert."
>      , "Alle overige principes worden als regel geinterpreteerd, wat inhoudt dat de informatiesystemen overtreding ervan onmogelijk maken."
>      , latexEnumerate ([addSlashes (explainRule ctx language r)|pat<-dc, r<-rules pat++signals pat, null (cpu r)]++
>                        [addSlashes (explainMult ctx language d)|d<-declarations dc, (not.null) (multiplicities d)])
>      , latexChapter "Kennisstructuur" "Kennisstructuur"
>      , "\tDe principes, zoals beschreven in hoofdstuk \\ref{chp:Principes} zijn via een ontologische analyse vertaald naar"
>      , "\teen kennisgraaf (figuur \\ref{fig:"++cname++"CD})."
>      , latexFigure (latexCenter ("  \\includegraphics[scale=.3]{"++cname++"_CD.png}")++
>        "\n\\caption{Kennisgraaf van "++cname++"}\n\\label{fig:"++cname++"CD}")
>      , "\nDe volgende tabel geeft een schatting van de omvang van de specificatie, uitgedrukt in functiepunten."
>      , "\nDeze schatting gaat uit van realisatie van uitsluitend de principes beschreven in hoofdstuk \\ref{chp:Principes}."
...
>      ["\\title{Conceptual Analysis\\\\ "++cname++"}"
>      , "\\maketitle"
>      , "\\tableofcontents"
>      , latexChapter "Introduction" "Introduction"
>      , "\tThis document defines core concepts in the context of "++name ctx++"."
...
>      , latexEnumerate ([addSlashes (explainRule ctx language r)|pat<-dc, r<-rules pat++signals pat, null (cpu r)]++
>                        [addSlashes (explainMult ctx language d)|d<-declarations dc, (not.null) (multiplicities d)])
>      , latexChapter "Knowledge Structure" "Knowledge Structure"
>      , "\tAn ontological analysis of the principles from the previous chapter (\\ref{chp:Principles}) has resulted in a knowledge graph,"
>      , "\twhich shown in figure \\ref{fig:"++cname++"CD}."
>      , latexFigure (latexCenter ("  \\includegraphics[scale=.4]{"++cname++"_CD.png}")++
>        "\n\\caption{Knowledge graph of "++cname++"}\n\\label{fig:"++cname++"CD}")
>      , "\tThe following table provides an estimate of the size of the specification, if it only realizes the principles described in chapter \\ref{chp:Principles}."
...
>   = if isSignal r then "Actie nemen wanneer "++xpln else xpln
>     where
>      xpln = if null (explain r)
...
>                                                    ++", maar niet voor elke "++(unCap.name.target) d++" hoeft er een "++(unCap.name.source) d++" te zijn in deze relatie."
>   | null ([Uni,Tot,    Sur] >- multiplicities d) = applyM d ("Elke "++(unCap.name.source) d) ("precies één "++(unCap.name.target) d)
>                                                    ++", maar elke "++(unCap.name.target) d++" mag gerelateerd zijn aan meerdere "++(unCap.plural Dutch .name.source) d++"."
...
>   | null ([    Tot,Inj    ] >- multiplicities d) = applyM d ("Elke "++(unCap.name.source) d) ("ten hoogste één "++(unCap.name.target) d)
>                                                    ++", terwijl elke "++(unCap.name.target) d++" in deze relatie is gekoppeld aan tenminste één "++(unCap.name.source) d++"."
...
>       (lIntro language++chain "\n"
>        [ "\\begin{document}"
>        , concat [lglos language c| c<-(rd' name . preCl . Cl context . wrld) context]
>        , "\\end{document}"
>        ])
...
>       (lIntro language++chain "\n"
>        [ "\\begin{document}"
>        , concat [lshow language c| c<-(rd' name . preCl . Cl context . wrld) context]
>        , "\\end{document}"
>        ])
