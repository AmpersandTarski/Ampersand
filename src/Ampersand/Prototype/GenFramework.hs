...
doGenFrontend ::
  (HasFSpecGenOpts env, HasRunner env, HasZwolleVersion env, HasDirPrototype env) =>
  FSpec ->
  RIO env ()
...
downloadPrototypeFramework ::
  (HasRunner env, HasZwolleVersion env, HasDirPrototype env) =>
  RIO env Bool
downloadPrototypeFramework =
  ( do
      env <- ask
      ...
      if x
        then do
          ...
          response <-
            (parseRequest url >>= httpBS) `catch` \err ->
              failWithMessage
  ...
  where
    ...
    extractionIsAllowed ::
      (HasProtoOpts env, HasLogFunc env) =>
      FilePath ->
      RIO env Bool
