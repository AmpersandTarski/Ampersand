...
doGenHaskell fSpec = do
  ...
  logDebug $ "Generating Haskell source code for " <> (display . text1ToText . tName) fSpec <> "..."
...
doGenSQLdump fSpec = do
  env <- ask
  outputFile <- outputFile' <$> ask
  logDebug $ "Generating SQL queries dumpfile for " <> (display . text1ToText . tName) fSpec <> "..."
  liftIO $ createDirectoryIfMissing True (takeDirectory outputFile)
