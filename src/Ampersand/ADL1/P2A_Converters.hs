...
import Ampersand.Basics hiding (conc, guard, join, set)
import Ampersand.Classes
import Ampersand.Classes.ConceptStructure (PConceptStructure (..))
import Ampersand.Core.AbstractSyntaxTree hiding (CTXE, Checked, CtxError, Errors, Guarded (..), Warning)
              ...
              case filter ((== refName) . name) (ctxifcs ctx) of
                ...
                [refIfc] ->
                  let refConcept = source (objExpression (ifcObj refIfc))
                   ...
                   in -- Check if expectedConcept (what we're passing) >= refConcept (what interface expects)
                      -- This ensures we're passing something at least as specific as what the interface needs
                      case refConcept `geq` parentConcept of
...
findRelsTyped declMap x tp =
  let result = Map.findWithDefault [] (SignOrd tp) (Map.map (: []) (findRels declMap x))
...
makePCpt2ACpt typologies = pCpt2aCpt
  where
    ...
    conceptTable =
      -- trace ("CONCEPT TABLE: " <> tshow (Map.keys conceptTableMap))
      conceptTableMap
      where
        conceptTableMap =
          Map.fromList
...
pCtx2aCtx
  ...
  PCtx
    ...
    where
      ...
      pCpts =
        pConcs (p_conceptdefs <> concatMap pt_cds p_patterns)
          `Set.union` pConcs (p_relations <> concatMap pt_dcs p_patterns)
          `Set.union` pConcs (p_gens <> concatMap pt_gns p_patterns)
          `Set.union` pConcs (p_rules <> concatMap pt_rls p_patterns)
          `Set.union` Set.fromList (concatMap (NE.toList . reprcpts) (p_representations <> concatMap pt_Reprs p_patterns))
          `Set.union` Set.fromList ([P_ONE] <> [PCpt nameOfSESSION | not (null p_interfaces)])
      ...
      getObjReprs interfaces viewdefs identdefs =
        case Set.toList (getInterfaceConcepts interfaces `Set.union` getViewConcepts viewdefs `Set.union` getIdentityConcepts identdefs) of
          [] -> []
          cpt : cpts ->
            -- trace ("8. getObjReprs "<>T.intercalate " " (NE.toList (fmap showA (cpt :| cpts))))
        ...
        where
          ...
          getInterfaceConcepts ifcs =
            Set.fromList [source (objExpression (ifcObj ifc)) | ifc <- ifcs]
              `Set.union` Set.unions [getObjectConcepts (ifcObj ifc) | ifc <- ifcs]
        ...
        where
          ...
          mkDuplicateReprTypeError pairs =
            CTXE (Origin "representation check")
              $ "Concept "
              <> (T.intercalate ", " . map showWithAliases . L.nub . map fst) pairs
      ...
      allReprs =
        p_representations
          <> concatMap pt_Reprs p_patterns -- P_Concept - TType bindings from REPRESENT statements outside of patterns
          <> identReprs -- P_Concept - TType bindings from REPRESENT statements inside patterns
          <> viewReprs -- implicit P_Concept - Object bindings for IDENT concepts
          -- implicit P_Concept - Object bindings for VIEW concepts
        where
        ...
        case sub of
          ...
          P_InterfaceRef
            { pos = orig,
              ...
              srcIfc <- getInterface -- get the source atom of the interface that is referenced, to check for compatibility.
              return
                ( InterfaceRef
                    { pos = orig,
                      siIsLink = isLink,
        ...
        case pBoxItem of
          P_BoxItemTerm
            { obj_PlainName = nm,
              ...
              a_msub <-
                if target objExpr == topCpt || target objExpr == botCpt
      ...
      typeCheckPairView ci expr (PairView lst) =
        ...
        where
          ...
          typeCheckPairViewSeg (PairViewExp segOrigin s x) =
            do
              let requiredConcept = (case s of Src -> source; Tgt -> target) expr
      ...
      pEnforce2aEnforce
        ...
        P_Enforce
          ...
          IsSuperSet {} ->
            do
              xpr <- term2Expr env ci Nothing (PInc pos' x relTerm)
              case xpr of
          ...
          IsSubSet {} ->
            do
              xpr <- term2Expr env ci Nothing (PInc pos' relTerm x)
              case xpr of
          ...
          IsSameSet {} ->
            do
              xpr <- term2Expr env ci Nothing (PEqu pos' relTerm x)
              case xpr of
...
showOpTree opTree =
  ...
  where
    ...
    paddings = map (\sigText -> T.replicate (maxWidth - T.length sigText + 1) " ") sigTexts
      where
        maxWidth = foldr (max . T.length) 0 sigTexts
...
mkVerboseTypeError env errorOrigin opTree baseMsg =
  Errors
    ...
    $ if logLevel (view runnerL env) <= LevelInfo
      then
        let treeOutput = showOpTree opTree
...
term2Expr env ci mConstraintCpt term =
  do
    sgnTree <- -- trace ("🔍 term2Expr called with constraint=" <> tshow mConstraintCpt <> ", term=" <> showP term <> " at " <> tshow (origin term)) $
      signatures (case mConstraintCpt of Just cpt -> Just (Src, cpt); Nothing -> Nothing) term
    checkAmbiguities sgnTree
    -- trace (showOpTree sgnTree)
    case sgnTree of
      ...
      _ ->
        ...
        do
          rootExpr <- rootExpression sgnTree
          ...
          let finalExpr =
                if isConcreteSignature (sign rootExpr)
          ...
          if isConcreteSignature (sign finalExpr)
            ...
            then pure finalExpr
              ...
              Errors
                . pure
                $ CTXE (origin term)
                $ "Cannot determine a concrete type for expression "
                <> showP term
                <> ". "
                <> "The inferred signature "
                <> tshow (sign finalExpr)
                <> " contains TOP or BOT. "
  ...
  where
      ...
      where
        ...
        findAmbiguities isRoot node =
          let triples = opSigns node
           ...
           in case triples of
                ...
                _ ->
                  if length sigs == length uniqueSigs
        ...
        makeAmbiguityError isRoot triples uniqueSigs =
          let trm = case triples of
           ...
           in CTXE (origin trm)
                $ "Ambiguity in "
                <> (if isRoot then "" else "sub")
                <> "expression "
                <> showP trm
                <> ".\n"
                <> "This "
                <> (if isRoot then "" else "sub")
                <> "expression can have signatures "
    ...
    signatures mConstraint trm = do
      opTree <- -- trace ("4. signatures ("<>tshow mConstraint<>") ("<>showP trm<>") in "<>tshow (origin trm)<>" yields:")
        resultTerm
      let refinedOpTree =
            -- trace ("   resultTerm:  "<>showOpTree opTree) $
      ...
      where
        ...
        resultPrim trmPrim =
          case trmPrim of
            ...
            PFlipped t -> do
              sgnTree <- signatures (fmap flp mConstraint) (Prim t)
              pure (flp sgnTree)
            PNamedR rel -> do
              relsList <- case p_mbSign rel of
                Just sg -> do
                  sgn <- pSign2aSign (conceptMap ci (origin trmPrim)) sg
        ...
        resultTerm =
          case trm of
            ...
            PFlp _ e -> do
              -- When flipping, swap the constraint using the Flippable instance
        ...
        errorsPeri o kind joinOrMeet combinator pCombinator a b sgnaTree sgnbTree sgnsa sgnsb =
          let errorOpTree = STbinary sgnaTree sgnbTree [(combinator (expr_a, expr_b), sgn_a, pCombinator trm_a trm_b) | (expr_a, sgn_a, trm_a) <- sgnsa, (expr_b, _, trm_b) <- sgnsb]
              ...
              getRootSig opTree = case opSigns opTree of
                [(_, sgn, _)] -> Just sgn
                _ -> Nothing
           in -- Check for ISgn types to provide better error messages
              case (getRootSig sgnaTree, getRootSig sgnbTree) of
                -- Special case: when one or both sides have ISgn (identity) types
                (Just (ISgn cptA), Just (Sign srcB tgtB))
                  | cptA == botCpt ->
                      mkVerboseTypeError
                        env
                        o
                ...
                (Just (Sign srcA tgtA), Just (ISgn cptB))
                  | cptB == botCpt ->
                      mkVerboseTypeError
                        env
                        o
        ...
        checkIntra o kind combinator pCombinator a b _opStr =
          -- activate the last parameter opStr when using the trace statements:
          do
            let lConstraint = case mConstraint of
                  Just (Src, _) -> mConstraint
                  _ -> Nothing
        ...
        constrain opTree =
          case mConstraint of
            Nothing -> pure opTree
            Just (sot, limit) ->
              -- ensure that the signatures in opTree are equal or wider than the constraint
              case [ case expr of
          ...
          where
            mkConstraintError o sot t expr limit =
              CTXE o
                ...
                <> showP t
                <> " has "
                <> (case sot of Src -> "source"; Tgt -> "target")
                <> " "
                <> tshow cpt
                <> ", which is "
                <> diagnosticString
