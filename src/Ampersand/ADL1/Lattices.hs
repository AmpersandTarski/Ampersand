...
data EqualitySystem a
  = ES
      (Map.Map a Int) -- whatever this is a system of
...
largestSubset i =
  ...
  where
    elimSubsets ((a, v) : as) = v : elimSubsets (filter (\x -> not (IntSet.isSubsetOf (fst x) a)) as)
    elimSubsets [] = []

endPoints :: (Ord a, SetLike x) => RevMap a -> [(IntSet.IntSet, x a)]
endPoints (RevMap st im) =
  if IntMap.null im then [(IntSet.empty, fromSet st) | not (slNull st)] else concatMap endPoints' (IntMap.toList im)
  where
    endPoints' (i, rm) = map addi (endPoints rm)
      where
        addi (lst, elm) = (IntSet.insert i lst, elm)
...
data RevMap a
  = RevMap
      (Set.Set a) -- all elements equivalent to this point in the map
...
optimize1 (ES oldmap oldimap) =
  ...
  where
    ...
    maybeMapper x =
      notEmpty
        [ (s1, imapTranslate oldimap s2 IntSet.empty)
          | (s1, s2) <- x,
            not (IntSet.null s1),
            not (IntSet.null s2)
        ]
...
addEquality' ~(ES nms imap) set1 set2 =
  ...
  where
    ...
    addRule oldimap origSet triggers newSet =
      ...
      where
        dif = IntSet.difference newSet origSet
