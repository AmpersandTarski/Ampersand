...
parseADL :: (HasExcellOutputOptions env, HasLogFunc env,HasLogFunc env) =>
...
parseFormalAmpersand :: (HasLogFunc env, HasExcellOutputOptions env) => RIO env (Guarded P_Context)
...
parseFormalAmpersandDocumented :: (HasLogFunc env, HasExcellOutputOptions env) => RIO env (Guarded P_Context)
...
parseSystemContext :: (HasLogFunc env, HasExcellOutputOptions env) => RIO env (Guarded P_Context)
...
parseThing :: (HasExcellOutputOptions env, HasLogFunc env,HasLogFunc env) =>
...
parseADLs :: (HasExcellOutputOptions env, HasLogFunc env,HasLogFunc env) =>
...
parseADLs parsedFilePaths fpIncludes =
  case fpIncludes of
    ...
    x:xs -> if x `elem` parsedFilePaths
        ...
        where parseTheRest :: (HasExcellOutputOptions env, HasLogFunc env,HasLogFunc env) =>
...
parseSingleADL :: (HasExcellOutputOptions env, HasLogFunc env,HasLogFunc env) =>
...
parseSingleADL pc
 = do case pcFileKind pc of
    ...
    where
     ...
     parseSingleADL' :: (HasExcellOutputOptions env) => RIO env (Guarded (P_Context, [ParseCandidate]))
     parseSingleADL'
         | extension == ".xlsx" =
