        ...
        pSA opts
         | extension == ".xlsx" =
             do { popFromExcel <- catchInvalidXlsx (parseXlsxFile (pcFileKind pc) filePath)
                ; pure ((\pops -> (mkContextOfPopsOnly pops,[])) <$> popFromExcel)  -- Excel file cannot contain include files
                }
         | genArchiAnal opts && extension == ".xml" = do
               ctxFromArchi <- doArchiAnalyze filePath  -- e.g. "CA repository.xml"
             --ctxFromArchi :: P_Context
               liftIO $ writeFile "ArchiMetaModel.adl" (prettyPrint ctxFromArchi)
               sayWhenLoudLn "ArchiMetaModel.adl written"
               sayWhenLoudLn (filePath ++ " has been interpreted as an Archi-repository.")
               pure ((\archiContents -> (archiContents,[])) <$> pure ctxFromArchi)  -- Excel file cannot contain include files
         | otherwise =
             do { mFileContents
                    <- case pcFileKind pc of
                       Just fileKind
