...
parseADL :: (HasParseOptions env, HasLogFunc env) =>
...
parseFormalAmpersand :: (HasParseOptions env, HasLogFunc env) => RIO env (Guarded P_Context)
...
parseFormalAmpersandDocumented :: (HasParseOptions env, HasLogFunc env) => RIO env (Guarded P_Context)
...
parseSystemContext :: (HasParseOptions env, HasLogFunc env) => RIO env (Guarded P_Context)
...
parseThing :: (HasParseOptions env, HasLogFunc env) =>
...
parseADLs :: (HasParseOptions env, HasLogFunc env) =>
...
parseADLs parsedFilePaths fpIncludes =
  case fpIncludes of
    ...
    x:xs -> if x `elem` parsedFilePaths
        ...
        where parseTheRest :: (HasParseOptions env, HasLogFunc env) =>
...
parseSingleADL :: (HasParseOptions env, HasLogFunc env) =>
...
parseSingleADL pc
 = do case pcFileKind pc of
    ...
    where
     ...
     parseSingleADL' :: (HasParseOptions env) => RIO env (Guarded (P_Context, [ParseCandidate]))
     parseSingleADL'
         | extension == ".xlsx" =
