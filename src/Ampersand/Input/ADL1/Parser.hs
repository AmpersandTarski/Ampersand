...
pFancyViewDef =
  mkViewDef
    ...
    <*> pBraces (pViewSegment False `sepBy` pComma)
    `opt` []
    <*> pMaybe pHtmlView
    <* pKey "ENDVIEW"
  where
...
pTerm =
  pTrm2
    <??> ( invertT PIsc
             <$> rightAssociate PIsc "/\\" pTrm2
             <|> invertT PUni
             <$> rightAssociate PUni "\\/" pTrm2
         )
...
pTrm4 =
  pTrm5
    <??> ( invertT PCps
             <$> rightAssociate PCps ";" pTrm5
             <|> invertT PRad
             <$> rightAssociate PRad "!" pTrm5
             <|> invertT PPrd
             <$> rightAssociate PPrd "#" pTrm5
         )
