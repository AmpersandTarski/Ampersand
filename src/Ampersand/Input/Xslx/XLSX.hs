...
parseXlsxFile mFk file =
 ...
 where
  ...
  toPops :: (HasFSpecGenOpts env) => env -> SheetCellsForTable -> [P_Population]
  toPops env x = map popForColumn (colNrs x)
    where
       ...
       where
         ...
         notEmpty k
            = case value k of
              ...
              Just (CellError _)  -> fatal $ "Error reading cell in file" <>T.pack file<>" sheet "<>sheetName<>" (row,col): "<>tshow k
              Nothing -> False
