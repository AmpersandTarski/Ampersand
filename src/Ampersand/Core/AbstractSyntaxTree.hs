...
data Declaration =
  Sgn { decnm :: Text                -- ^ the name of the declaration
      ...
      , dech :: Int                  -- ^ a hash code for this declaration. To be proven: dech is an injective function. I don't know the purpose of dech; please help...
      } |
  Isn
...
-- TODO: clarify the purpose of dech and reestablish the equality with dech, if deemed necessary.
...
instance Hashable Declaration where
  s `hashWithSalt` Sgn{dech = v} = s `hashWithSalt` v
  s `hashWithSalt` Isn{detyp = c} = s `hashWithSalt` c
  s `hashWithSalt` Vs{decsgn = sgn}  = s `hashWithSalt` sgn
instance Show Declaration where  -- For debugging purposes only (and fatal messages)
  showsPrec _ decl@Sgn{}  = showString (name decl++showSign (sign decl))
