...
plural English str =
  case T.uncons $ T.reverse str of
    ...
    _ -> case lookup str exceptions of
      ...
      Nothing -> str <> "s"
  where
    exceptions = [("child", "children"), ("Child", "Children"), ("mouse", "mice"), ("Mouse", "Mice"), ("sheep", "sheep"), ("Sheep", "Sheep")]
plural Dutch str =
  case T.uncons str of
    ...
    Just (h, tl) -> case matches of
      ...
      where
        ...
        matches =
          [ (T.reverse . T.drop (T.length s) . T.reverse) str <> p | (s, p) <- exceptions, (T.toLower . T.reverse . T.take (T.length s) . T.reverse) str == s
          ]
