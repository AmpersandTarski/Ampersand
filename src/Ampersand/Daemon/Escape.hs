...
wordWrapE mx gap = repeatedlyE f
  where
    f x =
      let (a, b) = splitAtE mx x
       in if b == Esc ""
            then ((a, WrapHard), Esc "")
            else
              let (a1, a2) = breakEndE isSpace a
               in if lengthE a2 <= gap then ((a1, WrapHard), app a2 b) else ((a, WrapSoft), trimStartE b)
