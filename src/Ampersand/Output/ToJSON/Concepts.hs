...
instance JSON MultiFSpecs Concepts where
 fromAmpersand env multi _ = Concepts (map (fromAmpersand env multi) (Set.elems $ concs fSpec))
   where fSpec = userFSpec multi
instance JSON A_Concept Concept where
 fromAmpersand env multi cpt = Concept
  ...
  , cptJSONconceptTable      = fromAmpersand env multi cpt
