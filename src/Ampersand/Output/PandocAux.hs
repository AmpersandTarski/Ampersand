...
instance ShowMath Expression where
  showMath = math . showExpr . insParentheses
    where
      ...
      showExpr (EDcD d) = inMathText . text1ToText . tName $ d
      showExpr (EDcI c) = "I_{ \\lbrack " <> (inMathText . text1ToText . tName) c <> " \\rbrack }"
      ...
      showExpr (EDcV sgn) = "V_{ \\lbrack " <> (inMathText . text1ToText . tName . source) sgn <> "*" <> (inMathText . text1ToText . tName . target) sgn <> " \\rbrack }"
...
instance ShowMath Relation where
  showMath decl =
    math . noBreaking $
      (inMathText . text1ToText . tName) decl <> " \\lbrack "
        <> (inMathText . text1ToText . tName . source) decl
        <> (if isFunction (EDcD decl) then " \\mapsto " else "*")
        <> (inMathText . text1ToText . tName . target) decl
        <> " \\rbrack "
