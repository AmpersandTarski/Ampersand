...
module Ampersand.Output.ToPandoc.ChapterIntroduction (chpIntroduction) where
...
chpIntroduction env now fSpec =
  ...
  where
    ...
    fSpecName = (singleQuoted . text . text1ToText . tName) fSpec
    readingGuide =
      case outputLang' of
        Dutch ->
          para
            ( text "Dit document"
                ...
                <> if SharedLang `elem` chaptersInDoc env
                  then
                    text "Deze afspraken staan opgesomd in "
            ...
            <> ( if ConceptualAnalysis `elem` chaptersInDoc env
                   then
                     para
                       ( text "De conceptuele analyse in "
        ...
        English ->
          para
            ( text "This document"
                ...
                <> if SharedLang `elem` chaptersInDoc env
                  then
                    text "Those rules are listed in "
                      <> hyperLinkTo SharedLang
            ...
            <> ( if ConceptualAnalysis `elem` chaptersInDoc env
                   then
                     para
                       ( text "The conceptual analysis in "
                           <> hyperLinkTo ConceptualAnalysis
