  ...
  where
    ...
    caSection themeContent
      |  null (cptsOfTheme themeContent)
         ...
         && null (idRulesOfTheme themeContent) = mempty
      | otherwise =
        --  *** Header of the theme: ***
      ...
      where
        ...
        themeClasses = case patOfTheme themeContent of
         ...
         where allClassesInPats = concatMap (classes . cdAnalysis False fSpec) (patterns fSpec)
        ...
        caSubsections =
          [ ( header 3 (str (name cl)) <> identityBlocks cpt <> entityBlocks,
            ...
            | (cl,cpt) <- entities, (entityBlocks, entityRels) <- [caEntity cl]
          ] <>
        ...
        identityText r
         = case rrkind r of
            Identity c -> (para . l) ( NL ("Een identiteit op \""<>name c<>"\" is gedefinieerd, zij het zonder PURPOSE.")
       ...
       in ( mempty,
            [ -- First the reason why the rule exists, if any..
              purp
                ...
                <> plain
                      ( l ( NL "Dit is - gebruikmakend van relaties ",
                            EN "Using relations "
                          )
                      <> mconcat
                        ( L.intersperse
                ...
                <> para
                  ( hyperLinkTo (pictOfRule r)
                      <> l ( NL " geeft een conceptueel diagram van deze regel.",
