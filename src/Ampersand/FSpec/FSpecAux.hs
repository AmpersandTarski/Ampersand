...
getRelationTableInfo fSpec dcl =
  case filter thisDcl . concatMap getRelInfos $ [p | InternalPlug p <- plugInfos fSpec] of
    [(p, store)] -> (p, store)
    [] -> fatal ("Relation not found: " <> (text1ToText . tName) dcl)
    _ -> fatal ("Relation found multiple times: " <> (text1ToText . tName) dcl)
  where
...
getConceptTableInfo fSpec cpt =
  case lookupCpt fSpec cpt of
    [] -> fatal ("No plug found for concept '" <> (text1ToText . tName) cpt <> "'.")
