...
import Ampersand.FSpec.ShowECA
 ...
 = SBdgrm
   ...
   where
     ...
     inEvNodes = [ DotNode { nodeID         = nameINode eventsIn ev
                           , nodeAttributes = [Label (StrLabel (fromString (show (eSrt ev)++" "++showA (eDcl ev))))]
                           }
                 | ev<-eventsIn
                 ]
     --DESCR -> All conjuncts
     ...
     ecaNodes  = [ DotNode { nodeID         = nameENode (fsbECAs fsb) eca
                           , nodeAttributes = if isBlk (ecaAction eca)
                                              ...
                                              else [(Label . StrLabel . fromString. showECA "") eca]
                           }
...
nmLkp :: (Eq a, Show a) => FSpec -> String -> [a] -> a -> String
nmLkp _ prefix xs x
 = head ([prefix++show (i::Int) | (i,e)<-zip [1..] xs, e==x]++
         fatal 216 ("illegal lookup in nmLkp "++show prefix++": " ++show x++
                    "\nin: ["++intercalate "\n    , " (map show xs)++"\n    ]")
        )
