...
data CrudInfo = CrudInfo
  { allCrudObjects :: [(A_Concept, [A_Concept])], -- crud concept together with its target concept in the surjective/total transitive closure of relations
...
mkCrudInfo allConceptsPrim decls allIfcs =
  ...
  where
    ...
    crudObjs =
      [ (crudCncpt, Map.findWithDefault [] crudCncpt transSurjClosureMap)
    ...
    getCrudObjsPerIfc ifc =
      [ (cObj, isC, isR, isU, isD)
        | cObj <- crudCncpts,
  ...
  where
    editableTarget expr =
      case getExpressionRelation expr of
        Nothing -> []
        Just (declSrc, decl, declTgt, isFlipped')
          | decl `elem` [] -> [(decl, if isFlipped' then declSrc else declTgt)]
          | otherwise -> []
...
getCrudObjsPerConcept crudsPerIfc = L.sortBy (compare `on` fst) conceptsAndInterfaces
  where
    conceptsAndInterfaces :: [(A_Concept, ([Interface], [Interface], [Interface], [Interface]))]
    conceptsAndInterfaces = concatMap toIfcPerConcept crudsPerIfc

    toIfcPerConcept ::
      (Interface, [(A_Concept, Bool, Bool, Bool, Bool)]) ->
      [(A_Concept, ([Interface], [Interface], [Interface], [Interface]))]
    toIfcPerConcept (ifc, ifcCrudObjs) =
      [ ( cncpt,
          ( [ifc | isC],
            [ifc | isR],
            [ifc | isU],
