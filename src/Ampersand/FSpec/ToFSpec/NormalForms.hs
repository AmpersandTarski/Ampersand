          ...
          [ DStep
            ...
            | (term, rewriteTerms) <- matchableRules,
              ...
              original == rCombinator ls
                || fatal
                  ( "When analysing rule " <> rd <> " with unifier " <> showIT unif <> " on:  " <> showIT (rCombinator ls)
                      <> "\nWe substitute:  "
      ...
      [ DStep
        ...
        | null [() | e <- Set.toList s, not (isValid e), fatal ("Invalid subexpr: " <> showIT e)],
          ...
          original == rCombinator s
            || fatal
              ( "When analysing rule " <> rd <> " with unifier " <> showIT unif <> " on:  " <> showIT (rCombinator s)
                  <> "\nWe substitute:  "
                  <> showIT original
                  <> "\nby:             "
                  <> showIT result
...
substitute ruleDoc unifier term =
  ...
  where
    ...
    subs (RVar r _ _) = case substExprs r of
      ...
      es -> fatal ("Rule:  " <> ruleDoc <> "\nVariable " <> (text1ToText . tName) r <> " in term " <> showIT term <> " has been bound to multiple expressions:\n   " <> T.intercalate "\n   " (map showIT es))
    subs (RId c) = case substExprs (name c) of
      [e] -> e -- This is e@(RId c')
      [] -> fatal ("Rule:  " <> ruleDoc <> "\nVariable " <> (text1ToText . tName) c <> " is not in term " <> showIT term)
      es -> fatal ("Rule:  " <> ruleDoc <> "\nVariable " <> (text1ToText . tName) c <> " in term " <> showIT term <> " has been bound to multiple expressions:\n   " <> T.intercalate "\n   " (map showIT es))
