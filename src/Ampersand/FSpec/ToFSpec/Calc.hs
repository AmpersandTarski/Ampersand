...
--            , commaPandocAnd ,commaPandocOr--TODO: this shouldt be here!
            , quadsOfRules
          --  , testInterface
            ) where
...
--    = "\nInterface "++ name ifc++"("++intercalate ", " [showA r++":"++name (target r) | r<-rels]++")\n"++
--      " - The parameters correspond to editable fields in a user interface.\n   "++
--      showA ifc++"\n"++
--      " - Invariants:\n   "++intercalate "\n   " [showA rule    | rule<-invs]++"\n"++
...
deriveProofs opts context
 = testConfluence context<>
   ...
   bulletList [ para ( "-- quad ------------"<>linebreak<>"When relation "<>(str . showA . qDcl) q<>" is changed,"
                       <>linebreak<>(str . showA . qRule) q
                       <>(if (length . qConjuncts) q<=1 then space else " ("<>(str . show . length . qConjuncts) q<>" conjuncts)")
                       <>" must be restored."<>linebreak<>"This quad has conjunct: "<>(str . showA . rc_conjunct) x
                       <>" and "<>(str.show.length.rc_dnfClauses) x<>" dnf clauses."
                     ) <>
                bulletList [ para (linebreak<>"Dnf clause "<>str (show dc)) | dc<-rc_dnfClauses x]
              | q<-quads, x<-qConjuncts q ] <>
   para (linebreak<>linebreak<>"Second step: assemble dnf clauses.") <>
   bulletList [ para ( "Dnf clause "<>str (showA dc)
                       <>linebreak<>"is derived from rule "<>str (showA r)
                       <>linebreak
                       <>case ms of
                            []    -> "No relations affect this clause."
                            [rel] -> "It can be called when relation " <>str (showA rel)<>" is affected."
                            _     -> "It can be called when relations "<>str (commaEng "or" [showA rel | rel<-ms])<>" are affected."
                     )
              | (ms,dc,r)<-
   ...
   bulletList [ para ( "-- ECA Rule "<>(str.show.ecaNum) ecarule<>" ---------")<>
                ...
                bulletList [ para (linebreak<>"delta expression"<>linebreak<>space<>str (showA d)
                                   <>linebreak<>"derivation:"
                                  )<>
                             (showProof (para.str.showA).dfProof) d<>  -- Produces its result in disjunctive normal form
                             para ("disjunctly normalized delta expression"<>linebreak<>(str.showA.disjNF opts) d)
...
{-
           ...
           interText [linebreak] [showA rule<> " yields\n"<>interText "\n\n"
                                   [ ["event = ", str (show ev), space, str (showA rel), linebreak ] <>
                                     [str (showA r<>"["<>showA rel<>":="<>showA (actSem opts ev (EDcD rel) (delta (sign rel)))<>"] = r'"), linebreak ] <>
                                     ["r'    = "] <> conjProof r' <> [linebreak ] <>
                                     ["viols = r'-"] <> disjProof (ECpl r') <> [ linebreak ] <>
                                     "violations, considering that the valuation of "<>showA rel<>" has just been changed to "<>showA (actSem opts ev (EDcD rel) (delta (sign rel)))<>
                                     "            "<>conjProof (ECpl r) <>"\n"<>
                                     "reaction? evaluate r |- r' ("<>(str.showA.conjNF opts) (notCpl r .\/. r')<>")"<>
                                     ...
                                     (if null (lambda ev (ERel rel ) r)
                                      then "lambda "<>showA rel<>" ("<>showA r<>") = empty\n"
                                      else -- for debug purposes:
                                           -- "lambda "<>show ev<>" "<>showA rel<>" ("<>showA r<>") = \n"<>(interText "\n\n".map showPr.lambda ev (ERel rel)) r<>"\n"<>
                                           -- "derivMono ("<>showA r<>") "<>show ev<>" "<>showA rel<>"\n = "<>({-interText "\n". map -}showPr.derivMono r ev) rel<>"\n"<>
           ...
           where e = rrexp rule
                 ...
                 (exx',_,_) = last prf
            --     conjProof = showProof (para.str.showA) . cfProof (getOpts fSpec)
                 disjProof = showProof (para.str.showA) . dfProof (getOpts fSpec)
--                 showPr    = showProof (para.str.showA)  -- hoort bij de uitgecommentaarde code hierboven...
       --TODO: See ticket #105
-}
...
derivMono expr -- preconditie van actie a
 ...
 where
  ...
  rule neg' pos' | isTrue neg' = Ru { rrnm  = ""
                                    ...
                                    , rrexp = pos'
                                    , rrmean = AMeaning
                                               [Markup Dutch   (string2Blocks ReST "Waarom wordt deze regel hier aangemaakt? (In Calc.hs, regel 402)")
                                               ,Markup English (string2Blocks ReST "Why is this rule created? (In Calc.hs, line 403)")]  --TODO Stef, gaarne de explanations aanvullen/verwijderen. Dank! Han.
                                    , rrmsg = []
                                    , rrviol = Nothing
                                    ...
                                    , r_env = ""
                                    , r_usr = Multiplicity
                                    , isSignal = fatal 336 $ "erroneous reference to isSignal in rule ("++showA neg'++") |- ("++showA pos'++")"
                                    }
                 | otherwise   = Ru { rrnm  = ""
                                    , rrfps = Origin "rule generated for not(isTrue neg') by Calc"
                                    , rrexp = neg' .|-. pos'
                                    , rrmean = AMeaning
                                               [Markup Dutch   (string2Blocks ReST "Waarom wordt deze regel hier aangemaakt? (In Calc.hs, regel 332)")
                                               ,Markup English (string2Blocks ReST "Why is this rule created? (In Calc.hs, line 333)")]  --TODO Stef, gaarne de explanations aanvullen/verwijderen. Dank! Han.
                                    , rrmsg = []
                                    , rrviol = Nothing
                                    , rrtyp = sign neg' {- (neg `meet` pos) -}
                                    , rrdcl = Nothing
                                    , r_env = ""
                                    , r_usr = Multiplicity
                                    , isSignal = fatal 352 $ "illegal reference to isSignal in rule ("++showA neg'++") |- ("++showA pos'++")"
                                    }
       ...
       case expr of
        ...
        where
            ...
            inter' e@EIsc{} = if and [sgn==sign f | f<-exprIsc2list e, isVar f e3]
                              ...
                              else fatal 532 ("signature error in inter'  "++show [(showA f,showSign (sign f)) | f<-exprIsc2list e, isVar f e3])
...
-- longstcomn determines the longest prefix common to all xs in xss.
 ...
 -- remainders determines the remainders.
  ...
  derivtext tOp "invert" e'' expr = sh tOp++showA e''++" means "++sh (inv tOp)++showA expr++"."
  derivtext tOp "mono"    e'' expr = "("++showA e''++"->"++showA expr++") is monotonous, so "++sh tOp++showA e''++" means "++sh tOp++showA expr++"."
...
assembleECAs options context editables
 ...
 --       , let relEq = [ q | q<-vquads fSpec, qDcl q==rel] -- Gather the quads with the same declaration (qDcl). A quad has a declaration (qDcl), a rule (qRule) and clauses qConjuncts
        ...
        , let acts = [ -- go through all the events that affect that clause:
                       ...
                       , para ("Let us analyse clause "<>str (showA expr)<>" from rule "<>(str . commaEng "and" . map (name.snd)) conjEqClass<>".")<>
                         para ("event = "<>str (show ev)<>space<>str (showA rel)<>" means doing the following substitution")<>
                         para (str (showA clause<>"["<>showA rel<>":="<>showA (actSem options ev (EDcD rel) (delta (sign rel)))<>"] = clause'"))<>
                         para ("clause' = "<>str (showA ex')<>
                               if clause'==ex'
                               then ", which is already in conjunctive normal form."<>linebreak
                               else ", which has conjunctive normal form: "<>linebreak<>str (showA clause')
                              )<>
                         ...
                         (showProof (para.str.showA). cfProof) notClau<>
                         para ("So, notClau has CNF: "<>str (showA viols )<>linebreak<>
                               ( if viols==viols'
                                 then "This expression is in disjunctive normal form as well."
                                 else str ("In DNF, notClau is:  "<>showA viols'<>".")))<>
                         ( if isTrue clause'
                           then para ("This result proves the absence of violations, so a reaction of doing nothing is appropriate."<>linebreak
                                ...
                                (showProof (para.str.showA). cfProof) (expr .|-. clause')
                                      ...
                                      ( if ev==Ins
                                        then str (showA (conjNF options negs))<>" into "<> str (showA (disjNF options poss))<>"."
                                        else str (showA (disjNF options poss))<>" into "<> str (showA (conjNF options negs))<>"."
                                      )<>linebreak<>"deltFr: ")<>
                                (showProof (para.str.showA). dfProof) deltFr<>
                         ...
                         {-     <> "To finish the analysis of case "<>str (show ev)<>space<>str (showA rel)
                                   <>", let us compute the contents of "<>str (showA toExpr)<>" after insertion of viols."<>linebreak
                                   <>
                                ( if length (nub [sign viols, sign viols', sign toExpr])>1
                                  then fatal 248 ("viols"<>showSign (sign viols) <>"   "<>showA viols <>"\n"<>
                                                  "viols'"<>showSign (sign viols')<>"  "<>showA viols'<>"\n"<>
                                                  "toExpr"<>showSign (sign toExpr)<>"  "<>showA toExpr)
                                  else if ev==Ins
                                  then (showProof (para.str.showA). cfProof options) (viols'.\/.toExpr)<>linebreak
                                  else (showProof (para.str.showA). dfProof options) (notCpl viols./\.toExpr)<>linebreak
                                ) -}
                         )
                       )
                    ...
                    -- conjEqClass <- eqCl fst [ (qConjuncts q, qRule q) | q<-relEq ]
                     ...
                     , let negs    = if (length.nub.map sign) (vee:antcs clause)>1
                                     then fatal 265 ("type inconsistencies in antcs: "++show (map showA (vee:antcs clause)))
                                     else foldr (./\.) vee (antcs clause)
                     , let poss    = if (length.nub.map sign) (vee:conss clause)>1
                                     then fatal 265 ("type inconsistencies in conss: "++show (map showA (vee:conss clause)))
                     ...
                     , let deltFr  = if sign poss/=sign negs
                                     then fatal 274 ("type inconsistencies in deltFr: "++showA clause)
                                     else if ev==Ins
...
{-        (_ , _)           -> fatal 767 ( "(Stef?) Non-exhaustive patterns in the recursive call\n"
                                    ++"doCod ("++showA deltaX++") -- deltaX\n      "++show tOp++"  -- tOp\n      ("++showA expr++") -- expr\n"++
                                      "within function\ndoCode "++show tOp'++"  -- tOp'\n       ("++showA expr1++") -- expr1\n       ("++showA delta1++") -- delta1\n"++
                                      concat
                                      [ "while trying to maintain conjunct "++showA conjunct++
