       ...
       where
          ...
          enrichIfc ifc
           = ifc{ ifcEcas = fst . assembleECAs opts context $ []
                , ifcConjuncts = makeIfcControls [] allConjs
                }
     ...
     step4a
      = let recur es
             = [ Obj { objnm    = showA t
                     ...
                     , objmView = Nothing
                     , objmsub  = Just (Box OriginUnknown (target t) Nothing (recur [ pth | (_:pth)<-cl, not (null pth) ]))
                     }
        ...
        [Ifc { ifcObj      = Obj { objnm    = name c
                                 ...
                                 , objmView = Nothing
                                 , objmsub  = Just (Box OriginUnknown c Nothing objattributes)
                                 }
             , ifcEcas     = fst (assembleECAs opts context params)
             , ifcConjuncts = makeIfcControls params allConjs
         ...
         = -- Trace added to demonstrate that this function takes VERY long.
           ...
           Act{ actRule   = rul
              ...
              , actPurp   = [Expl { explPos = OriginUnknown
                                  ...
                                  , explMarkup = Markup { amLang   = Dutch
                            ...
                            ,Expl { explPos = OriginUnknown
                                  , explObj = ExplRule (name rul)
                                  , explMarkup = Markup { amLang   = English
