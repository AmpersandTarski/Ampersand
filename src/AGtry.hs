...
import CC_aux 
         (  Architecture(Arch)
          ...
          , isMph
          , KeyDef(Kd)
          , KeyDefs, nr, multiplicities, declaration
          , inline
         )
...
-- multiple ctTrees may occur if contexts are used multiply in different ctTrees.
    trees = makeClassificationsF (name.fst) tuples++[Cl s []| s<-singles]
            where tuples = [ ((spc,(mG,mD)),(gen,(mG',mD')))
                           | (spc@(Ctx nm on isa world dc ms cs ks),(mG,mD))<-ctxs, o<-on
...
renumber ss = [ Sgn nm a b props prL prM prR cs pos nr sig | (nr,Sgn nm a b props prL prM prR cs pos _ sig)<-zip [1..] ss]
mergecontents ss = [ Sgn nm a b (rd [p|s<-cl,p<-multiplicities s])
                                prL prM prR
                                (sord [l|s<-cl,l<-contents s])
                                p nr sig
                   | (nr,cl)<-zip [1..] (eqClass (==) ss)
                   , Sgn nm a b props prL prM prR cs p _ sig <-
                       take 1 ([s|s@(Sgn _ _ _ _ prL prM prR _ _ _ _)<-cl
                                 , not (null (prL++prM++prR))]++cl)
                   ]

deriveMults ks sgs
 = [Sgn nm a b (props `uni` derived s) prL prM prR cs pos nr sig| s@(Sgn nm a b props prL prM prR cs pos nr sig)<-sgs]
   where derived s = (rd.concat) [if inline m then [Uni,Tot] else [Sur,Inj]| Kd pos lbl c ms<-ks,  m<-ms, declaration m==s]
...
type T_Context = (Classification (Context,(Gens,Declarations))) ->
                 ...
                 ((Context),([(Context,(Gens,Declarations))]),([String]))
-- cata
sem_Context :: (Context) ->
               (T_Context)
sem_Context ((Ctx (_nm) (_on) (_isa) (_world) (_dc) (_ms) (_cs) (_ks))) =
    (sem_Context_Ctx (_nm) (_on) (_isa) (_world) ((sem_Patterns (_dc))) ((sem_Declarations (_ms))) ((sem_ConceptDefs (_cs))) ((sem_KeyDefs (_ks))))
sem_Context_Ctx :: (String) ->
                   ...
                   (T_KeyDefs) ->
                   (T_Context)
sem_Context_Ctx (_nm) (_on) (_isa) (_world) (_dc) (_ms) (_cs) (_ks) (_lhs_ctxTree) (_lhs_ctxs) =
    let (_mD) =
        ...
        ( _dc_conDefs,_dc_keyDefs,_dc_mGen,_dc_morphisms,_dc_patterns,_dc_rawDecls,_dc_rnr,_dc_sErr,_dc_usedDecls) =
            (_dc (_genE) (1) (_mD))
        ( _ms_declarations,_ms_rawDecls,_ms_sErr) =
            (_ms (_genE) (_mD))
        ( _cs_conDefs) =
            (_cs )
        ( _ks_keyDefs,_ks_morphisms,_ks_sErr) =
            (_ks (_genE) (_mD))
    in  (put_gE _genE _cD
         ( Ctx _nm
               _on
               ...
               _keys)
        ,[( put_gE _genE _cD (Ctx _nm _on _inh [] _dc_patterns _mC _cs_conDefs _keys)
         , (_dc_mGen, _mC))]
        ,_dc_sErr ++ _ks_sErr
        )
-- Contexts ----------------------------------------------------
...
sem_Declaration ((Sgn (_nm) (_a) (_b) (_props) (_prL) (_prM) (_prR) (_content) (_morPos) (_nr) (_sig))) =
    (sem_Declaration_Sgn (_nm) ((sem_Concept (_a))) ((sem_Concept (_b))) (_props) (_prL) (_prM) (_prR) (_content) (_morPos) (_nr) (_sig))
sem_Declaration ((Vs (_g) (_s))) =
    (sem_Declaration_Vs ((sem_Concept (_g))) ((sem_Concept (_s))))
sem_Declaration_Iscompl :: (T_Concept) ->
                           (T_Concept) ->
                           (T_Declaration)
sem_Declaration_Iscompl (_g) (_s) (_lhs_gE) (_lhs_sDef) =
    let ( _g_concept,_g_nm) =
            (_g (_lhs_gE))
        ( _s_concept,_s_nm) =
            (_s (_lhs_gE))
    in  (Iscompl _g_concept _s_concept,"I",Iscompl _g_concept _s_concept,[])
sem_Declaration_Isn :: (T_Concept) ->
                       (T_Concept) ->
                       (T_Declaration)
sem_Declaration_Isn (_g) (_s) (_lhs_gE) (_lhs_sDef) =
    let ( _g_concept,_g_nm) =
            (_g (_lhs_gE))
        ( _s_concept,_s_nm) =
            (_s (_lhs_gE))
    in  (Isn _g_concept _s_concept,"I",Isn _g_concept _s_concept,[])
sem_Declaration_Sgn :: (String) ->
                       ...
                       (Bool) ->
                       (T_Declaration)
sem_Declaration_Sgn (_nm) (_a) (_b) (_props) (_prL) (_prM) (_prR) (_content) (_morPos) (_nr) (_sig) (_lhs_gE) (_lhs_sDef) =
    let (_msignat) =
            head ([ s
                  | s <- _lhs_sDef, _nm==name s, _a_concept==source s, _b_concept==target s]++
                  [error ("Missing "++showHS (Sgn _nm _a_concept _b_concept _props _prL _prM _prR _content _morPos 0 _sig)++" in AGtry.ag\n"++ show _lhs_sDef)])
        ( _a_concept,_a_nm) =
            (_a (_lhs_gE))
        ( _b_concept,_b_nm) =
            (_b (_lhs_gE))
    in  (_msignat
        ,_nm
        ,Sgn _nm _a_concept _b_concept _props _prL _prM _prR _content _morPos 0 _sig
...
sem_Declaration_Vs :: (T_Concept) ->
                      (T_Concept) ->
                      (T_Declaration)
sem_Declaration_Vs (_g) (_s) (_lhs_gE) (_lhs_sDef) =
    let ( _g_concept,_g_nm) =
            (_g (_lhs_gE))
        ( _s_concept,_s_nm) =
            (_s (_lhs_gE))
    in  (Vs _g_concept _s_concept,"V",Vs _g_concept _s_concept,[])
-- Declarations ------------------------------------------------
{-
   inherited attributes:
      gE                   : GenR
...
sem_Declarations_Cons :: (T_Declaration) ->
                         (T_Declarations) ->
                         (T_Declarations)
sem_Declarations_Cons (_hd) (_tl) (_lhs_gE) (_lhs_sDef) =
    let ( _hd_declaration,_hd_nm,_hd_rawDecl,_hd_sErr) =
            (_hd (_lhs_gE) (_lhs_sDef))
        ( _tl_declarations,_tl_rawDecls,_tl_sErr) =
            (_tl (_lhs_gE) (_lhs_sDef))
    in  (_hd_declaration: _tl_declarations,_hd_rawDecl: _tl_rawDecls,_hd_sErr ++ _tl_sErr)
sem_Declarations_Nil :: (T_Declarations)
sem_Declarations_Nil (_lhs_gE) (_lhs_sDef) =
    let 
    in  ([],[],[])
-- Expression --------------------------------------------------
{-
...
type T_Expression = (GenR) ->
                    ...
                    (Int) ->
                    (Declarations) ->
                    ((Rules),(Expression),(Morphisms),(Expression),(Int),([String]),([(Concept,Concept)]))
-- cata
...
sem_Expression_Cp (_e) (_lhs_gE) (_lhs_isign) (_lhs_pn) (_lhs_pos) (_lhs_rnr) (_lhs_sDef) =
    let ( _e_closRules,_e_expr,_e_morphisms,_e_raw,_e_rnr,_e_sErr,_e_signs) =
            (_e (_lhs_gE) (_lhs_isign) (_lhs_pn) (_lhs_pos) (_lhs_rnr) (_lhs_sDef))
    in  (_e_closRules,Cp _e_expr,_e_morphisms,Cp _e_raw,_e_rnr,_e_sErr,_e_signs)
...
sem_Expression_F (_ts) (_lhs_gE) (_lhs_isign) (_lhs_pn) (_lhs_pos) (_lhs_rnr) (_lhs_sDef) =
    let (_sgns) =
        ...
        ( _ts_closRules,_ts_exprs,_ts_morphisms,_ts_raw,_ts_rnr,_ts_sErr,_ts_signss) =
...
sem_Expression_Fi (_fs) (_lhs_gE) (_lhs_isign) (_lhs_pn) (_lhs_pos) (_lhs_rnr) (_lhs_sDef) =
    let (_sgns) =
        ...
        (_dis) =
            ...
            in Fi (                                    _fs_exprs                                    )
        ( _fs_closRules,_fs_exprs,_fs_morphisms,_fs_raw,_fs_rnr,_fs_sErr,_fs_signss) =
...
sem_Expression_K0 (_e) (_lhs_gE) (_lhs_isign) (_lhs_pn) (_lhs_pos) (_lhs_rnr) (_lhs_sDef) =
    let (_closmor) =
            let [(a,b)] = take 1 _lhs_isign
            in Mph (             (name.head.declarations) _e_expr) _lhs_pos [] (a,b) True (Sgn (             (name.head.declarations) _e_expr) a b [Trn,Rfx] "" "" "" [] _lhs_pos 0 True)
...
sem_Expression_K1 (_e) (_lhs_gE) (_lhs_isign) (_lhs_pn) (_lhs_pos) (_lhs_rnr) (_lhs_sDef) =
    let (_closmor) =
            let [(a,b)] = take 1 _lhs_isign
            in Mph (             (name.head.declarations) _e_expr) _lhs_pos [] (a,b) True (Sgn (             (name.head.declarations) _e_expr) a b [Trn] "" "" "" [] _lhs_pos 0 False)
        ( _e_closRules,_e_expr,_e_morphisms,_e_raw,_e_rnr,_e_sErr,_e_signs) =
            (_e (_lhs_gE) (_lhs_isign) (_lhs_pn) (_lhs_pos) (_lhs_rnr+1) (_lhs_sDef))
    in  (let [(a,b)] = take 1 _lhs_isign
         in Ru 'E'
...
sem_Expression_Tc (_c) (_lhs_gE) (_lhs_isign) (_lhs_pn) (_lhs_pos) (_lhs_rnr) (_lhs_sDef) =
    let ( _c_closRules,_c_expr,_c_morphisms,_c_raw,_c_rnr,_c_sErr,_c_signs) =
            (_c (_lhs_gE) (_lhs_isign) (_lhs_pn) (_lhs_pos) (_lhs_rnr) (_lhs_sDef))
    in  (_c_closRules,Tc _c_expr,_c_morphisms,Tc _c_raw,_c_rnr,_c_sErr,_c_signs)
...
type T_Expressions = (GenR) ->
                     ...
                     ((Rules),(Expressions),(Morphisms),(Expressions),(Int),([String]),([[(Concept,Concept)]]))
-- cata
...
sem_Expressions_Cons (_hd) (_tl) (_lhs_gE) (_lhs_isign) (_lhs_pn) (_lhs_pos) (_lhs_rnr) (_lhs_sDef) (_lhs_septets) =
    let (_hsgn) =
        ...
        ( _hd_closRules,_hd_expr,_hd_morphisms,_hd_raw,_hd_rnr,_hd_sErr,_hd_signs) =
            (_hd (_lhs_gE)
                 ...
                 (_lhs_rnr)
                 (_lhs_sDef))
        ( _tl_closRules,_tl_exprs,_tl_morphisms,_tl_raw,_tl_rnr,_tl_sErr,_tl_signss) =
            (_tl (_lhs_gE)
                 (if null _lhs_septets then _lhs_isign else
                 ...
                 (_lhs_sDef)
                 (_septs))
    in  (_hd_closRules ++ _tl_closRules,_hd_expr : _tl_exprs,_hd_morphisms ++ _tl_morphisms,_hd_raw: _tl_raw,_tl_rnr,_hd_sErr ++ _tl_sErr,_hd_signs: _tl_signss)
...
sem_Gen_G :: (T_Concept) ->
             (T_Concept) ->
             (T_Gen)
sem_Gen_G (_genus) (_spec) (_lhs_gE) =
...
type T_KeyDef = (GenR) ->
                (Declarations) ->
                ((KeyDef),(Morphisms),([String]))
-- cata
sem_KeyDef :: (KeyDef) ->
              (T_KeyDef)
sem_KeyDef ((Kd (_pos) (_lbl) (_e) (_ms))) =
    (sem_KeyDef_Kd (_pos) (_lbl) ((sem_Concept (_e))) ((sem_Morphisms (_ms))))
sem_KeyDef_Kd :: (FilePos) ->
                 (String) ->
                 (T_Concept) ->
                 (T_Morphisms) ->
                 (T_KeyDef)
sem_KeyDef_Kd (_pos) (_lbl) (_e) (_ms) (_lhs_gE) (_lhs_sDef) =
    let (_nulls) =
            [a | a <- _ksgn, null a]
        (_ambs) =
            [a | a <- _ksgn, length a>1]
        (_ksgn) =
            [ [Mph nm pos ats (if yin then (source s,target s) else (target s,source s)) yin (error "illegal reference to 'ksgn' in semantics of Kd of KeyDef")
              | s<-ss
              ,     yin && (source s `_lhs_gE` _e_concept || _e_concept `_lhs_gE` source s) ||
                not yin && (target s `_lhs_gE` _e_concept || _e_concept `_lhs_gE` target s) ]
            | (Mph nm pos ats _ yin _,ss) <- _ms_raw]
        ( _e_concept,_e_nm) =
            (_e (_lhs_gE))
        ( _ms_morphisms,_ms_raw,_ms_usedDecls) =
            (_ms (_lhs_gE) (take 1 ([(src,Anything)|src<-irredC _lhs_gE (map source (concat _ksgn))] ++ [(_e_concept, _e_concept)])) (_lhs_sDef))
    in  (Kd _pos _lbl _e_concept _ms_morphisms
        ,_ms_morphisms
        ,[ "10 in "++show _pos++"\n   "++
           "Ambiguous declaration for attribute " ++ commaEng "and" [name a|a<-as] ++ " in entity " ++ name _e_concept ++ ".\n"
         | as <- _ambs]++
         [ "11 in "++show _pos++"\n   "++
           "Attribute " ++ nm ++ " is incompatible with entity " ++ name _e_concept ++ ".\n"
         | (Mph nm pos ats _ yin _,ss) <- _ms_raw
         , if yin
           then null[source s|s<-ss, source s `_lhs_gE` _e_concept || _e_concept `_lhs_gE` source s]
           else null[target s|s<-ss, target s `_lhs_gE` _e_concept || _e_concept `_lhs_gE` target s]]
        )
-- KeyDefs -----------------------------------------------------
{-
   inherited attributes:
...
sem_KeyDefs_Cons :: (T_KeyDef) ->
                    (T_KeyDefs) ->
                    (T_KeyDefs)
sem_KeyDefs_Cons (_hd) (_tl) (_lhs_gE) (_lhs_sDef) =
    let ( _hd_kd,_hd_morphisms,_hd_sErr) =
            (_hd (_lhs_gE) (_lhs_sDef))
        ( _tl_keyDefs,_tl_morphisms,_tl_sErr) =
            (_tl (_lhs_gE) (_lhs_sDef))
    in  (_hd_kd : _tl_keyDefs,_hd_morphisms ++ _tl_morphisms,_hd_sErr ++ _tl_sErr)
sem_KeyDefs_Nil :: (T_KeyDefs)
sem_KeyDefs_Nil (_lhs_gE) (_lhs_sDef) =
    let 
    in  ([],[],[])
-- Morphism ----------------------------------------------------
{-
...
sem_Morphism_Mph (_nm) (_pos) (_atts) (_sgn) (_yin) (_u) (_lhs_gE) (_lhs_isign) (_lhs_sDef) =
    let (_s) =
        ...
        (_ats) =
            rd ([C a _lhs_gE as|C a _ as<- _atts]++[S a _lhs_gE as|S a _ as<- _atts])
        ( _u_declaration,_u_nm,_u_rawDecl,_u_sErr) =
            (_u (_lhs_gE) (_lhs_sDef))
    in  (_ats
        ,False
...
type T_Pattern = (GenR) ->
                 (Int) ->
                 (Declarations) ->
                 (([ConceptDef]),(KeyDefs),(Gens),(Morphisms),(Patterns),(Declarations),(Int),([String]),(Declarations))
-- cata
sem_Pattern :: (Pattern) ->
               (T_Pattern)
...
sem_Pattern_Pat (_nm) (_rules) (_gen) (_pms) (_cs) (_ks) (_lhs_gE) (_lhs_rnr) (_lhs_sDef) =
    let ( _rules_declarations,_rules_mGen,_rules_morphisms,_rules_rnr,_rules_rules,_rules_sErr,_rules_usedDecls) =
            (_rules (_lhs_gE) (_nm) (_lhs_rnr) (_lhs_sDef))
        ( _gen_list) =
            (_gen (_lhs_gE))
        ( _pms_declarations,_pms_rawDecls,_pms_sErr) =
            (_pms (_lhs_gE) (_lhs_sDef))
        ( _cs_conDefs) =
            (_cs )
        ( _ks_keyDefs,_ks_morphisms,_ks_sErr) =
            (_ks (_lhs_gE) (_lhs_sDef))
    in  (_cs_conDefs,_ks_keyDefs,rd [G g s| G g s <- _rules_mGen ++ _gen_list, g/=s],_rules_morphisms,[Pat _nm _rules_rules ( _rules_mGen ++ _gen_list) ( _pms_declarations ++ _rules_declarations ) _cs_conDefs _ks_keyDefs],_pms_rawDecls,_rules_rnr,_rules_sErr ++ _pms_sErr ++ _ks_sErr,_rules_usedDecls ++declarations _ks_morphisms)
-- Patterns ----------------------------------------------------
{-
   inherited attributes:
      gE                   : GenR
...
sem_Patterns_Cons (_hd) (_tl) (_lhs_gE) (_lhs_rnr) (_lhs_sDef) =
    let ( _hd_conDefs,_hd_keyDefs,_hd_mGen,_hd_morphisms,_hd_patterns,_hd_rawDecls,_hd_rnr,_hd_sErr,_hd_usedDecls) =
            (_hd (_lhs_gE) (_lhs_rnr) (_lhs_sDef))
        ( _tl_conDefs,_tl_keyDefs,_tl_mGen,_tl_morphisms,_tl_patterns,_tl_rawDecls,_tl_rnr,_tl_sErr,_tl_usedDecls) =
            (_tl (_lhs_gE) (_hd_rnr) (_lhs_sDef))
    in  (_hd_conDefs ++ _tl_conDefs,_hd_keyDefs ++ _tl_keyDefs,_hd_mGen ++ _tl_mGen,_hd_morphisms ++ _tl_morphisms,_hd_patterns ++ _tl_patterns,_hd_rawDecls ++ _tl_rawDecls,_tl_rnr,_hd_sErr ++ _tl_sErr,rd(_hd_usedDecls ++ _tl_usedDecls))
...
type T_Rule = (GenR) ->
              ...
              (Int) ->
              (Declarations) ->
              ((Rules),(Declarations),(Gens),(Morphisms),(Int),(Rules),([String]),(Declarations))
-- cata
sem_Rule :: (Rule) ->
            (T_Rule)
...
sem_Rule_Gc (_gluePos) (_m) (_expr) (_cpu) (_sgn) (_nr) (_pn) (_lhs_gE) (_lhs_pn) (_lhs_rnr) (_lhs_sDef) =
    let (_mors) =
        ...
        (_rs) =
            [ subExprCheck (Ru 'I' (Tm _m_morphism) _gluePos _expr_expr _cpu_raw "" (a,b) _nr _pn)
        ...
        (_closdecls) =
            rd [s| Ru c antc p cons cpu expla (a,b) n pn <- _expr_closRules, s<-declarations antc]
        (_ms) =
            irredT _lhs_gE
             (if null _expr_signs
        ...
        ( _expr_closRules,_expr_expr,_expr_morphisms,_expr_raw,_expr_rnr,_expr_sErr,_expr_signs) =
            (_expr (_lhs_gE) (_esign) (_lhs_pn) (_gluePos) (_lhs_rnr+length _rs) (_lhs_sDef))
        ( _cpu_closRules,_cpu_exprs,_cpu_morphisms,_cpu_raw,_cpu_rnr,_cpu_sErr,_cpu_signss) =
            (_cpu (_lhs_gE) ([]) (_lhs_pn) (_gluePos) (_expr_rnr) (_lhs_sDef) ([]))
    in  (_expr_closRules
        ,[]
        ,rd [G g s| (d,c) <- _expr_signs, m <- _m_usedDecls, G g s<-[G d (source m), G c (target m)], g/=s]
        ,_mors
        ,_expr_rnr
        ,[ Ru c antc pos expr cpu expla (a,b) (i + _lhs_rnr) _lhs_pn
         | ((Ru c antc pos expr cpu expla (a,b) _ _,cpuErrs), i)<-zip _rs [0..]]
        ,take 1
          (  [ "12 in "++show _gluePos++
...
sem_Rule_Ru (_c) (_antc) (_rulePos) (_cons) (_cpu) (_expl) (_sgn) (_nr) (_pn) (_lhs_gE) (_lhs_pn) (_lhs_rnr) (_lhs_sDef) =
    let (_r) =
        ...
        (_clrs) =
            if _c=='A'
            then _cons_closRules
            else _antc_closRules ++ _cons_closRules
        (_is) =
        ...
        ( _antc_closRules,_antc_expr,_antc_morphisms,_antc_raw,_antc_rnr,_antc_sErr,_antc_signs) =
            (_antc (_lhs_gE) (if null _is then _antc_signs else _is) (_lhs_pn) (_rulePos) (_lhs_rnr+1) (_lhs_sDef))
        ( _cons_closRules,_cons_expr,_cons_morphisms,_cons_raw,_cons_rnr,_cons_sErr,_cons_signs) =
        ...
        ,take 1
         (_cons_sErr ++
           (if _c=='A' then [] else _antc_sErr ++
            ...
            | isIdent _antc_expr && or [s/=t| (s,t) <- _is] ]++
            [ "16 in "++show _rulePos++
            ...
            | isIdent _cons_expr && or [s/=t| (s,t) <- _is] ]++
...
sem_Rule_Sg (_pos) (_rule) (_expl) (_sgn) (_nr) (_pn) (_signal) (_lhs_gE) (_lhs_pn) (_lhs_rnr) (_lhs_sDef) =
    let (_signalname) =
            if null _signal_nm then "Rule"++show _lhs_rnr else _signal_nm
        (_decls) =
            [ Sgn _signalname a b [] "" "" "" [] _pos _lhs_rnr True
            | (a,b) <- map sign _rule_rules]
        ( _rule_closRules,_rule_declarations,_rule_mGen,_rule_morphisms,_rule_rnr,_rule_rules,_rule_sErr,_rule_usedDecls) =
            (_rule (_lhs_gE) (_lhs_pn) (_lhs_rnr) (_lhs_sDef))
        ( _signal_declaration,_signal_nm,_signal_rawDecl,_signal_sErr) =
            (_signal (_lhs_gE) (_lhs_sDef))
    in  (_rule_closRules
        ,_decls
        ,[]
        ,_rule_morphisms
        ,_rule_rnr
        ,[ Sg p r expla sgn n pn (head _decls)
         | r@(Ru c antc p cons cpu expla sgn n pn)<- _rule_rules ]
        ,_rule_sErr
...
sem_Rules_Cons (_hd) (_tl) (_lhs_gE) (_lhs_pn) (_lhs_rnr) (_lhs_sDef) =
    let ( _hd_closRules,_hd_declarations,_hd_mGen,_hd_morphisms,_hd_rnr,_hd_rules,_hd_sErr,_hd_usedDecls) =
    ...
    in  (_hd_declarations ++ _tl_declarations,_hd_mGen ++ _tl_mGen,_hd_morphisms ++ _tl_morphisms,_tl_rnr,_hd_rules ++ _hd_closRules ++ _tl_rules,_hd_sErr ++ _tl_sErr,_hd_usedDecls ++ _tl_usedDecls)
