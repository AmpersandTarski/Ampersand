...
subExprCheck r@(Ru c antc p cons cpu expla sgn nr pn)
 = (Ru c antc p cons matches expla sgn nr pn
   ...
   where
    ...
    recur  []    = ([],"")
    recur (e:es) = (subexps++matches , "   recur "++chain " " (map showADL (e:es))++" = "++chain " " (map showADL (subexps++matches)) ++"\nbecause\n"++
                   "("++chain " " (map showADL subexps)++" = subexprs ("++showADL e++") and\n"++
                   "("++chain " " (map showADL matches)++" = recur ("++chain " " (map showADL es)++")\n--------\n"++str'++str'')
     where (subexps,str')  = subexprs e
           (matches,str'') = recur es
    subexprs e@(Tm m)    = ([e| c<-r_cpu r, e `match` c] , "")
    subexprs e@(Tc e')   = subexprs e'
    subexprs e@(F ts)    = ([e| c<-r_cpu r, e `match` c]++matches ,  chain " " (map showADL matches)++" = recur ("++chain ";" (map showADL ts)++")\n--------\n"++str)
                           where (matches,str) = recur ts
    subexprs e@(Fd ts)   = ([e| c<-r_cpu r, e `match` c]++matches ,  chain " " (map showADL matches)++" = recur ("++chain "!" (map showADL ts)++")\n--------\n"++str)
                           where (matches,str) = recur ts
    subexprs e@(Fi fs)   = ([e| c<-r_cpu r, e `match` c]++matches ,  chain " " (map showADL matches)++" = recur ("++chain "/\\" (map showADL fs)++")\n--------\n"++str)
                           where (matches,str) = recur fs
    subexprs e@(Fu fs)   = ([e| c<-r_cpu r, e `match` c]++matches ,  chain " " (map showADL matches)++" = recur ("++chain "\\/" (map showADL fs)++")\n--------\n"++str)
                           where (matches,str) = recur fs
    subexprs e@(K0 e')   = ([e| c<-r_cpu r, e `match` c]++matches ,  chain " " (map showADL matches)++" = subexprs ("++showADL e'++")\n--------\n"++str)
                           where (matches,str) = subexprs e'
    subexprs e@(K1 e')   = ([e| c<-r_cpu r, e `match` c]++matches ,  chain " " (map showADL matches)++" = subexprs ("++showADL e'++")\n--------\n"++str)
                           where (matches,str) = subexprs e'
    subexprs e@(Cp e')   = ([e| c<-r_cpu r, e `match` c]++matches ,  chain " " (map showADL matches)++" = subexprs ("++showADL e'++")\n--------\n"++str)
                           where (matches,str) = subexprs e'
...
sem_KeyDef_Kd (_pos) (_lbl) (_ctx) (_ats) (_lhs_gE) (_lhs_rnr) (_lhs_sDef) =
    ...
    in  ([ expr | Obj nm pos expr ats strs <- _ats_objDefs],Kd _pos _lbl _ctx_expr _ats_objDefs,_ctx_rnr,_ats_rules ++ _ctx_rules,_ats_sErr)
-- KeyDefs -----------------------------------------------------
{-
   inherited attributes:
      gE                   : GenR
...
sem_ObjectDef :: (ObjectDef) ->
                 (T_ObjectDef)
sem_ObjectDef ((Obj (_nm) (_pos) (_ctx) (_ats) (_strs))) =
    (sem_ObjectDef_Obj (_nm) (_pos) ((sem_Expression (_ctx))) ((sem_ObjectDefs (_ats))) (_strs))
sem_ObjectDef_Obj :: (String) ->
                     ...
                     ([[String]]) ->
                     (T_ObjectDef)
sem_ObjectDef_Obj (_nm) (_pos) (_ctx) (_ats) (_strs) (_lhs_gE) (_lhs_iConc) (_lhs_iConcs) (_lhs_rnr) (_lhs_sDef) =
    let (_concpt) =
            if null _signs
    ...
    in  (_ats_objDefs
        ...
        ,if and [sgn==(Anything,Anything)|sgn<- _ctx_signs]
         then Obj _nm _pos (Tm (mIs (C _nm _lhs_gE []))) _ats_objDefs _strs
         else Obj _nm _pos _ctx_expr _ats_objDefs _strs
        ,_pos
        ,_ctx_rnr
...
-}
{-
   local variables for ObjectDefs.Cons:
      signs

-}
{-
   local variables for ObjectDefs.Nil:
...
sem_ObjectDefs :: (ObjectDefs) ->
                  (T_ObjectDefs)
sem_ObjectDefs (list) =
    (foldr (sem_ObjectDefs_Cons) (sem_ObjectDefs_Nil) ((map sem_ObjectDef list)))
sem_ObjectDefs_Cons :: (T_ObjectDef) ->
                       (T_ObjectDefs) ->
                       (T_ObjectDefs)
sem_ObjectDefs_Cons (_hd) (_tl) (_lhs_gE) (_lhs_iConc) (_lhs_iConcs) (_lhs_rnr) (_lhs_sDef) =
    let (_signs) =
...
sem_ObjectDefs_Nil (_lhs_gE) (_lhs_iConc) (_lhs_iConcs) (_lhs_rnr) (_lhs_sDef) =
    let 
...
sem_Rule_Gc (_gluePos) (_m) (_expr) (_cpu) (_sgn) (_nr) (_pn) (_lhs_gE) (_lhs_pn) (_lhs_rnr) (_lhs_sDef) =
    let (_rs) =
        ...
        (_ruls) =
            [ subExprCheck (Ru Implication (Tm _m_morphism) _gluePos _expr_expr _cpu_raw "" (a,b) _nr _lhs_pn)
        ...
        (_rcpu) =
            ...
            then Ru _c_typ (error ("Reference to antecedent of Truth Rule "++"showHS  r")) _rulePos _cons_expr _cpu_raw _expl
                    (if null _is
                     ...
                     _lhs_rnr _lhs_pn
            else Ru _c_typ _antc_expr _rulePos _cons_expr _cpu_raw _expl
                    (if null _is
...
-}
{-
   local variables for RuleType.Automatic:

-}
{-
   local variables for RuleType.Equivalence:

-}
{-
   local variables for RuleType.Generalization:

-}
{-
   local variables for RuleType.Implication:
