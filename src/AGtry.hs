...
sem_Rule_Ru (_c) (_antc) (_rulePos) (_cons) (_cpu) (_expl) (_sgn) (_nr) (_pn) (_lhs_gE) (_lhs_pn) (_lhs_rnr) (_lhs_sDef) =
    let (_r) =
        ...
        (_rcpu) =
            subExprCheck
            (if _c==AlwaysExpr
            then Ru _c_typ (error ("Reference to antecedent of AlwaysExpr Rule "++showHS "" _r)) _rulePos _cons_expr _cpu_raw _expl
                    (if null _is
        ...
        (_is) =
            if _c==AlwaysExpr then _cons_signs else
            if null _antc_signs
        ...
        ( _cons_expr,_cons_morphisms,_cons_raw,_cons_rnr,_cons_rules,_cons_sErr,_cons_signs) =
            (_cons (_lhs_gE) (if null _is then _cons_signs else _is) (_lhs_pn) (_rulePos) (if _c==AlwaysExpr then _lhs_rnr + 1 else _antc_rnr) (_lhs_sDef))
        ( _cpu_exprs,_cpu_morphisms,_cpu_raw,_cpu_rnr,_cpu_rules,_cpu_sErr,_cpu_signss) =
            (_cpu (_lhs_gE) (_is) (_lhs_pn) (_rulePos) (_cons_rnr) (_lhs_sDef) ([]))
    in  ([]
        ,[]
        ,(if _c==AlwaysExpr then [] else _antc_morphisms)++ _cons_morphisms
        ,_cons_rnr
        ,_rs
        ,take 1
         (_cons_sErr ++
           (if _c==AlwaysExpr then [] else _antc_sErr ++
            [ "8 in "++show _rulePos++"\n   Meaningless rule.\n"
