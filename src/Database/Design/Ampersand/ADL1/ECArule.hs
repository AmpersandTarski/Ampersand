...
eventsFrom p@New{}          = On Ins (Isn (paCpt p)): eventsFrom (paCl p [PAVString (Origin "New in `eventsFrom`.") "x"])
eventsFrom p@Rmv{}          = On Del (Isn (paCpt p)): eventsFrom (paCl p [PAVString (Origin "New in `eventsFrom`.") "x"])
eventsFrom Nop{}            = []
eventsFrom Blk{}            = []
