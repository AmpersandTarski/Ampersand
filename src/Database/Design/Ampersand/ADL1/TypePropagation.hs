...
module Database.Design.Ampersand.ADL1.TypePropagation (
 -- * Exported functions
...
import Database.Design.Ampersand.Basics (name, isc, fatalMsg)
...
data Type = TypExpr (Term TermPrim) SrcOrTgt | TypInCps (Term TermPrim) | TypInObjDef P_ObjectDef -- term is deriving Ord
...
showType t
 = case t of
     TypExpr (Prim (Pid _ c)) _             -> "pop ("++name c++") "
     TypExpr term@(Prim (PVee o)) sORt      -> codOrDom sORt++" ("++showADL term++") "++"("++ show o++")"
     TypExpr term@(Prim (Pfull _ _ _)) sORt -> codOrDom sORt++" ("++showADL term++")"
...
instance Prelude.Ord Type where -- first we fix all forms of I's, which satisfy r = r~.
  compare (TypExpr (Prim (Pid _ c))      _) (TypExpr (Prim (Pid _ c'))       _) = Prelude.compare c c'
  compare (TypExpr (Prim (Pid _ _))      _) (TypExpr _                       _) = Prelude.LT
  compare (TypExpr _                     _) (TypExpr (Prim (Pid _ _ ))       _) = Prelude.GT
  compare (TypExpr (Prim (Patm _ x (Just c))) _) (TypExpr (Prim (Patm _ x' (Just c'))) _) = Prelude.compare (x,c) (x',c')
  compare (TypExpr (Prim (Patm _ _ (Just _))) _) (TypExpr (Prim (Patm _ _        _  )) _) = Prelude.LT
  compare (TypExpr (Prim (Patm _ _       _ )) _) (TypExpr (Prim (Patm _ _  (Just _ ))) _) = Prelude.GT
  compare (TypExpr (Prim (Patm o x Nothing )) _) (TypExpr (Prim (Patm o' x' Nothing )) _) = Prelude.compare (o,x) (o',x')
  compare (TypExpr (Prim (Patm _ _ _       )) _) (TypExpr (Prim (Patm _  _  _       )) _) = fatal 76 "Patm should not have two types"
  compare (TypExpr (Prim (Patm _ _ _       )) _) (TypExpr _                            _) = Prelude.LT
  compare (TypExpr _                          _) (TypExpr (Prim (Patm _ _ _))          _) = Prelude.GT
  -- note that V = V~ does not hold in general
  compare (TypExpr (Prim (PVee o))       x) (TypExpr (Prim (PVee o'))       x') = Prelude.compare (o,x) (o',x') -- This is a V of which the type must be determined (by the environment).
  compare (TypExpr (Prim (PVee _))       _) (TypExpr _                       _) = Prelude.LT
  compare (TypExpr _                     _) (TypExpr (Prim (PVee _))         _) = Prelude.GT
  -- here we implement V[A*B] = V[B*A]~ directly in the TypExpr
  compare (TypExpr (Prim (Pfull _ s t))  x) (TypExpr (Prim (Pfull _ s' t')) x') = Prelude.compare (if x==Src then (s,t) else (t,s)) (if x'==Src then (s',t') else (t',s')) -- This is a V of which the type is determined by the user
  compare (TypExpr (Prim (Pfull _ _ _))  _) (TypExpr _                       _) = Prelude.LT
  compare (TypExpr _                     _) (TypExpr (Prim (Pfull _ _ _))    _) = Prelude.GT
...
p_flp :: Term TermPrim -> Term TermPrim
p_flp a@(Prim (PI{}))       = a
p_flp a@(Prim (Pid{}))      = a
p_flp a@(Prim (Patm{}))     = a
p_flp (Prim (Pfull o s t))  = Prim (Pfull o t s)
p_flp (PFlp _ a)   = a
p_flp a            = PFlp (origin a) a

decToTyp :: SrcOrTgt -> P_Declaration -> Type
decToTyp b d = TypExpr (Prim (PNamedR (PNamedRel (origin d) (dec_nm d) (Just (dec_sign d))))) b
...
makeDecisions inp trnRel eqtyps
 ...
 where inpTrnRel = composeMaps trnRel inp
       ...
       isConc (TypExpr (Prim (Pid _ _)) _) = True
       isConc _ = False
       f :: Type -> [Type]
...
typing :: Typemap -> [Between] -> Map String [P_Declaration]
          -> ( Typemap                    -- st               -- the st relation: 'a st b' means that  'dom a' is a subset of 'dom b'
             ...
             , Guarded ( Map (Term TermPrim) P_Declaration -- bindings   -- declarations that may be bound to relations
...
--  checkBetween ::  ->
 ...
 -- The purpose of stClosAdd is to enhance a type map with the edges from Between nodes.
    ...
    declByTerm :: Map (Term TermPrim) [P_Declaration]
    declByTerm
      = Map.fromList ( [ (o, dbn) 
                       | o@(Prim (PNamedR (PNamedRel _ s Nothing))) <- allTerms
                       , let dbn = Map.findWithDefault [] s declsByName
                       ] ++
                       [ (o,filter ((==) sgn . dec_sign) $ dbn)
                       | o@(Prim (PNamedR (PNamedRel _ s (Just sgn)))) <- allTerms
                       , let dbn = Map.findWithDefault [] s declsByName
                       ]
                     )
    
    ivTypByTyp :: Map Type [P_Concept] -> Map Type [(Type,P_Concept,Type)]
    ivTypByTyp ivMap = Map.fromListWith mrgUnion [ (tp,map (\c -> (tp,c,TypExpr (Prim (Pid (fatal 313 "Hopefully this isn't inspected") c)) Src)) concs)
                                                 | (tp,concs) <- Map.toList ivMap ]
    
    typByTyp :: Map (Term TermPrim) [P_Declaration] -> Map Type [(Term TermPrim,P_Declaration,Type)]
    ...
    bindings :: Map (Term TermPrim) P_Declaration
    ...
    isaClos' = Map.fromDistinctAscList [(c,[c' | TypExpr (Prim (Pid _ c')) _<-ts,c'/=c]) | (TypExpr (Prim (Pid _ c)) _, ts)<-Map.toAscList stClos]
    ...
    stConcepts =  Map.map f stClos
                  where f :: [Type] -> [P_Concept]
                        ...
                        ownTypes ts = [c | TypExpr (Prim (Pid _ c)) _<-ts]
