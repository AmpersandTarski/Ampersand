...
import Data.Char
import Database.Design.Ampersand.Output.PandocAux (latexEscShw,texOnly_Id,singletonValueAsString)
...
assemble expr
 = case (source expr, target expr) of
  ...
  where
   ...
   f _ (EMp1 val _) _             = Atom . showADL . singletonValueAsString $ val
...
-}
   ...
   relFun exclVars lhs e rhs
     = case e of
         ...
         EMp1 val _      -> \_ _-> Atom . showADL . singletonValueAsString $ val
