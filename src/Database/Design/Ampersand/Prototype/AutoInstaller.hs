...
import Database.Design.Ampersand.Prototype.Installer (plug2tbl,dropplug,TableSpec,sessionTableSpec,historyTableSpec)
...
odbcinstall fSpec dsn =
   do verboseLn (getOpts fSpec) ("Connecting to ODBC connection "++ dsn ++"...")
      ...
      _ <- creates conn (historyTableSpec : sessionTableSpec : [plug2tbl p |InternalPlug p<-plugInfos fSpec])
