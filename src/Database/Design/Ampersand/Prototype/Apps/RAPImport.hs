...
makeFailedPops imperr script opts usrfiles
...
--options for file names and user name -> file names in the upload directory of the user
...
makeRAPPops fSpec usrfiles pics
 = let -- savepopfile is a SavePopFile (only POPULATIONS) which must be INCLUDEd to compile
   ...
   in
    ...
    ,makepopu ("ptrls","Pattern","Rule")        [(prcid p      , ruleid r)        | p<-vpatterns fs, r<-rules (fpProc p)]
    ,makepopu ("ptrls","Pattern","Rule")        [(prcid p      , ruleid r)        | p<-vpatterns fs, d<-relsDefdIn (fpProc p),decusr d, pr<-rapmults d, let r=rulefromProp pr d]
    ,makepopu ("ptgns","Pattern","Gen")         [(prcid p      , genid g)         | p<-vpatterns fs, g<-gens (fpProc p)]
    ,makepopu ("ptdcs","Pattern","Declaration") [(prcid p      , decid d)         | p<-vpatterns fs, d<-relsDefdIn (fpProc p),decusr d]
    ,makepopu ("ptxps","Pattern","Blob")        [(prcid p, nonsid (aMarkup2String (explMarkup ex)))
                                                                                  | p<-vpatterns fs, ex<-explanations fs, explForObj (fpProc p) (explObj ex)]
