 ...
 where
     ...
     enrichIfc ifc
      = ifc{ ifcEcas = fst (assembleECAs fSpec editables)
           , ifcControls = mkIfcControls (ifcParams ifc) allConjs
           }
...
--                student theme => generate interface for each concept with relations where concept is source or target (note: step1-3 are skipped)
     ...
     step4a
      ...
      = [Ifc { ifcClass = Nothing
             ...
             , ifcControls = mkIfcControls params allConjs
...
mkIfcControls :: [Expression] -> [Conjunct] -> [Conjunct]
mkIfcControls params allConjs = [ conj 
                                | conj<-allConjs
                                , (not.null) (params `isc` primsMentionedIn (rc_conjunct conj))
                                , not (ruleIsInvariantUniOrInj $ rc_orgRule conj)
                                ]
  where ruleIsInvariantUniOrInj :: Rule -> Bool
