...
--  fields p@TblSQL{}
--   = [ Obj { objnm    = attName tFld
--           , objpos   = Origin "This object is generated by fields (Object PlugSQL)"
--           , objctx   = if source a==concept p then a  else f (source a) [[a]]
--           , objcrud  = fatal 138 "Cruds are not defined for objects in TblSQL"
--           , objmView = Nothing
--           , objmsub  = Nothing
--           , objstrs  = []
--           }
--      | (a,_,tFld)<-mLkpTbl p] -- HJO,[20160317] TODO: are fields still in use? For what? If so, should the cLkpTbl be used too? 
--     where
--      f c mms
