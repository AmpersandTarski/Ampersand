...
>                  [ quote(conceptname c)++" -> "++quote(conceptname p)++" [color=red,style=dotted]"
>                  | (p,c)<-isas ]
...
>        arcs = rd ([m'|m<-mors pat++mors(specs pat), isMph m, not (isSgnl (declaration m)), not (isProperty m)
>                   {- , take 5(name m)/="Clos_"  -}
>                      , m'<-[m,flp m], inline m']++
>                   [Mph (name d) posNone [source d,target d] (source d,target d) True d
>                      | d<-declarations pat, not (isSgnl d), not (isProperty d)])
>        introG = "\n   { bgcolor=transparent  \n"
>              ++ "   ; node [shape=plaintext,fontsize=12,font=helvetica]\n"
>              ++ "   ; edge [len=1.2,fontsize=12,arrowsize=0.8]"
>

         *** Tekenen van Concepten ***
        De concepten worden getekend als een bolletje. Daarnaast wordt er een 'onzichtbaar'
        node getekend, die de naam van het concept met zich meedraagt. Een edge tussen het punt
        en het onzichtbare punt zorgt voor de verbinding.
...
>        onzichtbaarlijntje percent = " [style=invis,len=" ++ show(0.4 * percent / 100) ++ "] "
...
>                         "]"
>
>        relatielabel m = " [shape=plaintext,style=bold" ++
>                         ",fontsize=12,font=helvetica" ++
>                         ",label=" ++ quote (name m) ++
...
>                                            point 2 py   ++ newline ++
>                                            point 2 pz   ++ newline ++
>                                            line 1 pa py ++ newline ++
>                                            line 2 py px ++ newline ++
>                                            line 3 px pz ++ newline ++
>                                            line 4 pz pb
>                   | otherwise            = newline ++
>                                            point 1 px   ++ newline ++
>                                            line 5 pa px ++ newline ++
>                                            line 6 px pb
>             where
...
>                line s p1 p2 = quote p1 ++ edgearrow ++ quote p2 ++ linestyle s
...
>                linestyle s | s == 1 = constructstyle (defcolor "red") geen staart
>                            | s == 2 = constructstyle (defcolor "green") pijl geen
>                            | s == 3 = constructstyle (defcolor "blue") geen lijp
>                            | s == 4 = constructstyle (defcolor "orange") kop geen
>                            | s == 5 = constructstyle (defcolor "yellow") pijl staart
>                            | s == 6 = constructstyle (defcolor "purple") kop lijp
>                   where
...
>   dotGraph context style nm (Ctx cnm on isa world dc ms cs ks)
