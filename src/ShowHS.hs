   ...
   instance ShowHS Fservice where
    ...
    showHS flags indent fservice
     = "Fservice "
       ...
       ++ "\n-- The relations that may be changed by the user of this service:"
       ...
       where delt=error("!Fatal (module ShowHS 284): illegal reference to argument of ECA rule")
   ...
   instance ShowHS Field where
    ...
    showHS flags indent fld
     = "Att "++       "{ fld_name     = "++                     show (fld_name     fld)
       ++ ( if null (fld_sub fld)
            then indent++"    , fld_sub      = []"
            else indent++"    , fld_sub      = "++showHS flags (indent++"      ") (fld_sub  fld) )
...
--    showHSname context = haskellIdentifier ("ctx_"++name context)
...
--       (if null ks   then " []" else indent++"       "++showL [showHSname k       | k<-ks  ])++
