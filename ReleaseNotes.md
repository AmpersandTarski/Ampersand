...
## v5.6.2
- **A session has at most one active role**: the framework relation `PrototypeContext.sessionActiveRoles` is now declared `[UNI]`, so a session has at most one active role at any moment. This matches the prototype framework's role switcher, which activates exactly one role and deactivates the others, presenting roles as a single-choice list rather than independently toggled roles. The access-control rule "Active roles MUST be a subset of allowed roles" is unchanged. The embedded `PrototypeContext` is regenerated into the compiler on build, so every prototype built with this version inherits the single-active-role behaviour.
